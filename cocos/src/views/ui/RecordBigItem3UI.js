// 3人战绩一条item
var RecordBigItem3UI = RecordBigItem4UI.extend({

	update_base_info: function (parent, round_result, user_info_list, itemRoot, doSubStr) {
		...
		for (var i = 0; i < user_info_list.length; i++) {
			let player_label = itemRoot.getChildByName("player_label" + i.toString());
	...
	update: function (rootUINode, data) {
		...
		let total_score_list = [];
		this.update_base_info(rootUINode, round_result, user_info_list, rootUINode.getChildByName("scroll_items"));

		// this.getDataIter(user_info_list)

		var dataIter = cutil.simpleIterWithoutNull(round_result[0]["round_record"]);
		var itemRoot = rootUINode.getChildByName("scroll_items");
		itemRoot.setTouchEnabled(dataIter.size() > 4);
		for (var i = 0; i < user_info_list.length; i++) {
			let score_label = itemRoot.getChildByName("score_label" + i.toString());
			let player_label = itemRoot.getChildByName("player_label" + i.toString());
			let itemData = dataIter.next();
			if (!itemData) {
				player_label.setVisible(false);
			...
			total_score_list.push(total_score);
			this.update_score_color(score_label, total_score)
		}
		if (data["game_round"] > 9999) {
			lackrecord_label.setVisible(true);
			for (var i = 0; i < user_info_list.length; i++) {
				if (total_score_list[i] <= data["gameMaxLose"] * (-1)) {
					lackrecord_label.setVisible(false);
					break;
				}
			}
		} else {
