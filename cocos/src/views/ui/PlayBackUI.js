...
var PlayBackUI = MultipleLayoutUI.extend({
    ...
    showOperationPanel: function (serverSitNum, ops, doOp) {
	    ...
	    index = cutil.change_idx(player.curGameRoom.player_num, index);
    ...
    hideOperationPanel:function () {
        let player = h1global.player();
        for (var i = 0; i < player.curGameRoom.player_num; i++) {
            var cur_player_operation_panel = this.rootUINode.getChildByName('player_operation_panel' + i);
            cur_player_operation_panel.stopAllActions();
            cur_player_operation_panel.setVisible(false);
        }
    },
