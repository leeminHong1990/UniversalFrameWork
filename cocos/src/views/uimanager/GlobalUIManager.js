var GlobalUIManager = UIManagerBase.extend({
	onCreate:function(){
		var initUIClassNameList = ["InfoUI", "BroadcastUI", "LockUI","SuperInfoUI"];
        ...
        if(switches.show_version){
            if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){
                ...
                version_label.setPosition(cc.p(0, 0));
                version_label.runAction(cc.callFunc(function () {
                    if (cc.sys.isObjectValid(version_label)) {
                        version_label.setPosition(cc.winSize.width, 0)
                    }
                }));
