"use strict";
var DTLGFMJPlaybackOperationAdapter = DTLGFMJGameRules.extend({
	...
	preprocessGameRoom: function (roomInfo) {
        ...
        this.curGameRoom.hint_list = cutil.deepCopy(roomInfo['hint_tiles']);
		this.curGameRoom.op_record_list = JSON.parse(roomInfo['op_record_list']);
		this.curGameRoom.leftTileNum  -= roomInfo['hint_tiles'][dealerIdx].length + roomInfo["dealer_tiles_num"] - 14;
		...
		for (var i = 0; i < init_tiles.length; i++) {
			this.curGameRoom.handTilesList[i] = init_tiles[i].sort(cutil_dtlgfmj.tileSortFunc);
			if (i == dealerIdx) {
