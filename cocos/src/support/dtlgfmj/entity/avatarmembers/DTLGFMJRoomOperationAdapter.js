"use strict";
var DTLGFMJRoomOperationAdapter = GameOperationAdapter.extend({
	...
	handleReconnect: function (recRoomInfo) {
		...
		if (handTiles.length % 3 === 2 && final_op === const_val.OP_DRAW) {
			...
			this.curGameRoom.handTilesList[this.serverSitNum] = handTiles.sort(cutil_dtlgfmj.tileSortFunc);
			this.curGameRoom.handTilesList[this.serverSitNum].push(finalDrawTile);
		} else {
			this.curGameRoom.handTilesList[this.serverSitNum] = handTiles.sort(cutil_dtlgfmj.tileSortFunc);
		}
        this.curGameRoom.last_op = final_op;
