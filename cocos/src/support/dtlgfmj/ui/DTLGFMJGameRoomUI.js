...
"use strict"
var DTLGFMJGameRoomUI = UIBase.extend({
	ctor:function() {
		this._super();
		this.talk_img_num = 0;
	},
	initUI:function(){
		...
		this.init_hint_operation_panel();
		...
		for (var i = 0; i < player.curGameRoom.player_num; i++) {
			this.update_hint_tiles_panel(i, player.curGameRoom.hint_list[i]);
    ...
    _resetSelectTile: function () {
        ...
        if (this.curSelectTile) {
            ...
            this.update_canwin_tile_panel(const_dtlgfmj.NOT_DISPLAY_CANWIN_PANEL);
        }
    },
	...
	init_player_tile_panel:function(){
        ...
        function unchoose_tile() {
            ...
            self.update_canwin_tile_panel(const_dtlgfmj.NOT_DISPLAY_CANWIN_PANEL);
        }
        ...
        function can_choose_tile(tile) {
            let player = h1global.player();
            if (player && player.curGameRoom.curPlayerSitNum === player.serverSitNum && !player.gameOperationAdapter.canDiscard(tile.tileNum)){
                return false;
            }
            return true;
        }
        ...
        function hand_tile_event(tile, touch_mode, sender){
            ...
            if(!can_choose_tile(tile)){return;}
		...
		function touch_func(touch_pos, eventType, sender){
			...
			// cc.log("touch_pos",eventType, touch_pos, effective_width, handTiles.length, handTiles);
    ...
    mark_king_tile:function (tile_img) {
        tile_img.color = const_dtlgfmj.mark_king_color;
    },
    ...
    mark_discard_king_tiles:function () {
        ...
        for(var i = 0; i < list.length; i++){
            ...
            let start = 0 ;
            if(tileSum > const_dtlgfmj.MAX_DISCARD_TILES_SIZE){
                start = tileSum - const_dtlgfmj.MAX_DISCARD_TILES_SIZE;
            }
    ...
    mark_same_tile:function(tile_img){
        tile_img.color = const_dtlgfmj.mark_same_color;
    },
	...
	cancel_same_tile:function(tile_img){
		tile_img.color = const_dtlgfmj.mark_none_color;
	},
	...
	cancel_mark_same_tiles:function(){
		...
		for(var i = 0; i < 4; i++){
			...
			for(var j = 0; j< 20; j++){
				...
				if(player.curGameRoom.kingTiles.indexOf(tile_img.tile_num) >= 0){
					this.mark_king_tile(tile_img)
				}
			}
		}
	...
	startBeginAnim:function(startTilesList, diceList, dealerIdx){
		...
		this.lock_player_hand_tiles();
		this.show_hint_tiles_panel(false);
		this.show_hint_operation_panel(false);
		var self = this;
		...
		function playAnimation(){
				...
				cc.Sequence.create(cc.DelayTime.create(0.2),
					cc.CallFunc.create(function(){
						...
						for(var i = 0; i < curGameRoom.player_num; i++){
							...
							self.update_player_hand_tiles(i, startTilesList[i].slice(0, cur_tile_num), true);
						}
			...
			var step3 = cc.CallFunc.create(function(){
				...
				//显示手牌
				if (curGameRoom.dealerIdx == myServerSitNum) {
					curGameRoom.handTilesList[curGameRoom.dealerIdx] = curGameRoom.realHandTiles;
				}
				self.show_begin_bright_tile();
				for(var i = 0; i < curGameRoom.player_num; i++){
					self.update_player_hand_tiles(i, curGameRoom.handTilesList[i]);
					var cur_player_tile_panel = self.rootUINode.getChildByName("player_tile_panel" + i )
					var cur_player_hand_panel = cur_player_tile_panel.getChildByName("player_hand_panel");
					cur_player_hand_panel.setVisible(true);
					self.update_hint_tiles_panel(i, curGameRoom.hint_list[i]);
				}
				...
				if(myServerSitNum == curPlayerSitNum && (curGameRoom.handTilesList[myServerSitNum].length)%3==2){
					...
					self.init_hint_operation_panel();
				} else {
	...
	show_begin_bright_tile:function () {
		if (!this.is_show) { return; }
		var player = h1global.player();
		if (player.curGameRoom.hint_list[player.curGameRoom.dealerIdx].length > 0) {
			h1global.curUIMgr.roomLayoutMgr.notifyObserver("playOperationEffect", const_dtlgfmj.OP_LIANG, player.curGameRoom.dealerIdx);
		}
	},
    ...
    _createKingTileMark: function (kingtilemarkOptions, idx, isLiang) {
	...
	update_player_hand_tiles: function (serverSitNum, tileList, ignoreMark) {
        if (!this.is_show) { return; }
        var player = h1global.player();
        var idx = player.server2CurSitNum(serverSitNum);
        var cur_player_tile_panel = this.rootUINode.getChildByName("player_tile_panel" + idx.toString()).getChildByName("player_hand_panel");
        ...
        for (var i = 0; i < 14; i++) {
            ...
            if(handTilesList[i] !== undefined){
                if(handTilesList[i] > 0){
                    ...
                    if(!ignoreMark){
                        if(player.gameOperationAdapter.canPassiveHint()){
                            this.cancel_same_tile(tile_img);
                            this.mark_same_tile(tile_img);
                        } else if(player.gameOperationAdapter.opLimit(handTilesList[i]) && handTilesList.length%3==2){
                            this.cancel_same_tile(tile_img);
                            this.mark_same_tile(tile_img);
                        } else {
        ...
        for (var i = 0; i < curUpNum; i++) {
            for (var j = 0; j < 3; j++) {
                ...
                if (cur_upTilesList[i][j]) {
                    tile_img.tile_num = cur_upTilesList[i][j];
            ...
            if (cur_upTilesList[i].length > 3) {
                ...
                if (cur_upTilesList[i][3]) {
                    ...
                    mahjong_img.setVisible(true);
                    if (player.curGameRoom.kingTiles.indexOf(cur_upTilesList[i][3]) >= 0) {
                        var kingtilemark_img = this._createKingTileMark(kingtilemarkOptions, i);
                        this.upTileMarksList[idx].push(kingtilemark_img);
	...
	play_discard_anim:function(serverSitNum, onlyTilePointer){
        ...
        if(discardTilesSum > const_dtlgfmj.MAX_DISCARD_TILES_SIZE) discardIndex = const_dtlgfmj.MAX_DISCARD_TILES_SIZE - 1;
	...
	update_player_discard_tiles:function(serverSitNum){
		...
		var deskSize = const_dtlgfmj.DISCARD_TILES_SIZE;
		var maxDeskSize = const_dtlgfmj.MAX_DISCARD_TILES_SIZE;
        ...
        for(var i = 0; i < deskSize; i++){
			var tile_img = cur_player_discard_panel.getChildByName("tile_img_" + i.toString());
            if(i < maxDeskSize && discardTilesList[i] > 0){
                ...
                tile_img.setVisible(true);
                if(player.curGameRoom.kingTiles.indexOf(discardTilesList[i]) >= 0){
	...
	init_operation_panel:function(){
		...
		if(player.curGameRoom.waitAidList.length > 0){
			...
			if (player.curGameRoom.last_op == const_val.OP_DRAW){
                this.update_operation_panel(player.gameOperationAdapter.getDrawOpDict(player.curGameRoom.lastDrawTile), const_val.SHOW_DO_OP)
			}else {
                this.update_operation_panel(player.gameOperationAdapter.getPongKongOpDict(), const_val.SHOW_DO_OP)
			}
    ...
    update_wintips_btn:function () {
        ...
        if(canWinTiles.length <= 0 || player.curGameRoom.curPlayerSitNum == player.serverSitNum){
            ...
            wintips_btn.addTouchEventListener(function (sender, eventType) {
                if (eventType === ccui.Widget.TOUCH_BEGAN) {
                    self.update_canwin_tile_panel(const_dtlgfmj.WINTIPS_BTN_DISPLAY);
    ...
    update_canwin_tile_panel:function(select_tile){
        ...
        var self = this;
        var canwin_tile_panel = self.rootUINode.getChildByName("canwin_tile_panel");
        if(select_tile == const_dtlgfmj.NOT_DISPLAY_CANWIN_PANEL){
            canwin_tile_panel.setVisible(false);
            return;
        }
        var player = h1global.player();
    ...
    playOperationEffect:function(opId, serverSitNum, tile){
        ...
        if(opId == const_val.OP_PONG){
            ...
            this.playOperationFunc(curSitNum, "kong");
        } else if(opId == const_dtlgfmj.OP_LIANG){
	        this.playOperationFunc(curSitNum, "liang");
	...
	play_luckytiles_anim: function (callback, luckyTileList) {
        var player = h1global.player();
		var show_lucky_tiles_panel = this.rootUINode.getChildByName("show_lucky_tiles_panel");
        var panel_width = 100 * (luckyTileList.length - 1) + (player.curGameRoom.lucky_num > 0 ? 120 : 100);
		...
		for(var i = 0; i < 2; i++){
			...
			var mahjong_img = tile_img.getChildByName("mahjong_img");
			var lucky_num = luckyTileList[i];
			if(lucky_num){
				tile_img.setPositionX(60 + 100 * i);
				tile_img.setVisible(true);
				tile_img.loadTexture("Mahjong/mahjong_tile_player_down8.png", ccui.Widget.PLIST_TEXTURE);
				mahjong_img.setVisible(false);
				tile_img.runAction(cc.Sequence.create(cc.DelayTime.create(0.4 * (i + 1)), cc.CallFunc.create((function(lucky_num, tile_img, mahjong_img){return function(){
													tile_img.loadTexture("Mahjong/mahjong_tile_player_hand.png", ccui.Widget.PLIST_TEXTURE);
													mahjong_img.ignoreContentAdaptWithSize(true);
													mahjong_img.loadTexture("Mahjong/mahjong_big_" + lucky_num.toString() + ".png", ccui.Widget.PLIST_TEXTURE);
													mahjong_img.setVisible(true);
												};})(lucky_num, tile_img, mahjong_img))));
			} else {
				tile_img.setVisible(false);
			}
		}
    ...
    play_result_anim: function (player_info_list) {
        ...
        for (var i = 0; i < curGameRoom.player_num; i++) {
            ...
            var handTiles = cutil.deepCopy(player_info.tiles).sort(cutil_dtlgfmj.tileSortFunc);
    ...
    update_operation_panel:function(op_dict, from_type){
		...
		this.temp_operation_btn_list = operation_btn_list;
	...
	show_hint_tiles_panel:function (visible) {
    	var player = h1global.player();
    	for (var i = 0 ; i < player.curGameRoom.player_num ; i++) {
		    var cur_player_discard_panel = this.game_info_panel.getChildByName("player_discard_panel" + i.toString());
		    var hint_tiles_panel = cur_player_discard_panel.getChildByName("hint_tiles_panel");
		    hint_tiles_panel.setVisible(visible);
	    }
	},

	update_hint_tiles_panel:function (serverSitNum, tiles) {
		if (!this.is_show) {return;}
		var player = h1global.player();
		var idx = player.server2CurSitNum(serverSitNum);
		var cur_player_discard_panel = this.game_info_panel.getChildByName("player_discard_panel" + idx.toString());
		...
		for(var i = 0; i < this.liangTileMarksList[idx].length; i++){
			this.liangTileMarksList[idx][i].removeFromParent();
		}
		this.liangTileMarksList[idx] = [];
		var hint_tiles_panel = cur_player_discard_panel.getChildByName("hint_tiles_panel");
		if (tiles.length <= 0) {
			hint_tiles_panel.setVisible(false);
			return;
		} else {
			hint_tiles_panel.setVisible(true);
		}
		var options = this.get_player_discard_tiles_config(idx);
		// var hint_num_list = [0,0,0,0,0,0,0];
		// for (var i = 0 ; i < 7 ; i++) {
		// 	for (var j = 0 ; j < tiles.length; j++) {
		// 		if (const_dtlgfmj.WINDS_AND_DRAGONS[i] == tiles[j]) {
		// 			hint_num_list[i]++;
		// 		}
		// 	}
		// }
		for (var k = 0 ; k < 30 ; k++) {
			var tile_img = hint_tiles_panel.getChildByName("tile_img_" + k.toString());
			if (!tile_img) {cc.log("not tile_img");continue;}
			if (!tiles[k]) {cc.log("not tiles[k]");tile_img.setVisible(false);continue;}
			tile_img.setVisible(true);
			var mahjong_img = tile_img.getChildByName("mahjong_img");
			mahjong_img.ignoreContentAdaptWithSize(true);
			mahjong_img.loadTexture("Mahjong/mahjong_big_" + tiles[k] + ".png", ccui.Widget.PLIST_TEXTURE);
			mahjong_img.setScale(0.5);
			if (options.mahjong) {
				if (options.mahjong.flippedX !== undefined) {mahjong_img.setFlippedX(options.mahjong.flippedX);}
				if (options.mahjong.flippedY !== undefined) {mahjong_img.setFlippedY(options.mahjong.flippedY);}
				if (options.mahjong.flippedX_liang_list) {mahjong_img.setFlippedX(options.mahjong.flippedX_liang_list[(k + 14) % 14]);}
				if (options.mahjong.scale) {mahjong_img.setScale(options.mahjong.scale[0], options.mahjong.scale[1]);}
				if (options.mahjong.rotationX_liang) {mahjong_img.setRotationX(options.mahjong.rotationX_liang[(k + 14) % 14]);}
				if (options.mahjong.skewY) {mahjong_img.setSkewY(options.mahjong.skewY);}
			} else {
				cc.log("not mahjong params");
			}
			if (player.curGameRoom.kingTiles.indexOf(tiles[k]) >= 0) {
				if(options.kingtilemark){
					var kingtilemark_img = this._createKingTileMark(options.kingtilemark, (k + 14) % 14, true);
					this.liangTileMarksList[idx].push(kingtilemark_img);
					tile_img.addChild(kingtilemark_img);
					this.cancel_same_tile(tile_img);
					this.mark_king_tile(tile_img);
				}else{
	...
	init_hint_operation_panel:function () {
    	var player = h1global.player();
		if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow() && !player.gameOperationAdapter.canPassiveHint()) {
			if(player.gameOperationAdapter.canActiveHint()){
				h1global.curUIMgr.roomLayoutMgr.notifyObserver( "update_hint_operation_panel");
			}
		}
	},

	show_hint_operation_panel:function (visible) {
		this.rootUINode.getChildByName("hint_operation_panel").setVisible(visible);
	},

	update_hint_operation_panel:function () {
		if (!this.is_show) {return;}
		var self = this;
		var player = h1global.player();
		var kingTile = player.curGameRoom.kingTiles[0];
		var handTiles = player.curGameRoom.handTilesList[player.serverSitNum];
		var windsTileNum = 0;
		for (var n = 0 ; n < handTiles.length; n++) {
			if (handTiles[n] > const_val.BOUNDARY) {
				windsTileNum++;
			}
		}
		if (const_dtlgfmj.WINDS_AND_DRAGONS.indexOf(kingTile) < 0 && windsTileNum < 3) {
			cc.log("kingTile is not in const_dtlgfmj.WINDS_AND_DRAGONS! or windsTileNum < 3!");
			return;
		}
		var kingNum = cutil_dtlgfmj.get_count(handTiles, kingTile);
		if (const_dtlgfmj.WINDS_AND_DRAGONS.indexOf(kingTile) < 0) {
			kingNum = 0;
		}
		if (windsTileNum === kingNum) {return;}
		var hint_operation_panel = this.rootUINode.getChildByName("hint_operation_panel");
		hint_operation_panel.setVisible(false);
		hint_operation_panel.setTouchEnabled(false);
		var inum = 0;
		var isWin = false;
		for(var j = 0 ; j < this.temp_operation_btn_list.length ; j++) {
			if (this.temp_operation_btn_list[j].op_val === const_val.SHOW_WIN) {
				isWin = true;
			}
		}
		var isOne = false;
		for(var i = 0 ; i < 5 ; i++) {
			let bright_tiles_btn = hint_operation_panel.getChildByName("bright_tiles_btn_" + i.toString());
			if ((kingNum >= 1 && i === 0 && windsTileNum === 3) || (windsTileNum === kingNum && windsTileNum >= 3 && i === 0)) {
				hint_operation_panel.setVisible(true);
				bright_tiles_btn.setVisible(true);
			} else if (kingNum > 1 && i > 0 && i <= kingNum && windsTileNum > 3 && !(windsTileNum === kingNum && windsTileNum >= 3) && !isOne) {
				bright_tiles_btn.setVisible(true);
				hint_operation_panel.setVisible(true);
				bright_tiles_btn.setPositionY(55);
				if (hint_operation_panel.getChildByName("bright_tiles_btn_0").isVisible()) {
					inum --;
				}
				hint_operation_panel.getChildByName("bright_tiles_btn_0").setVisible(false);
			} else {
				bright_tiles_btn.setVisible(false);
			}
			if (i === 1 && windsTileNum - kingNum === 1) {bright_tiles_btn.setVisible(false);}
			if (i === 0 && windsTileNum - kingNum >= 3) {
				hint_operation_panel.setVisible(true);
				bright_tiles_btn.setVisible(true);
				isOne = true;
			}
			if (bright_tiles_btn.isVisible()) {
				inum ++;
				hint_operation_panel.getChildByName("pass_bright_tiles_btn").setPosition(cc.p(hint_operation_panel.getContentSize().width - 40, hint_operation_panel.getContentSize().height/2));
				hint_operation_panel.getChildByName("pass_bright_tiles_btn").setVisible(true);
				if (this.temp_operation_btn_list.length > 0 && isWin) {
					this.show_operation_panel();
					bright_tiles_btn.setPosition(cc.p(hint_operation_panel.getContentSize().width - 40 -150 * inum - (this.temp_operation_btn_list.length - 1) * 150, hint_operation_panel.getContentSize().height/2));
				} else {
					if(h1global.curUIMgr.roomLayoutMgr) {
						h1global.curUIMgr.roomLayoutMgr.notifyObserver('hide_operation_panel');
					}
					bright_tiles_btn.setPosition(cc.p(hint_operation_panel.getContentSize().width - 40 -150 * inum, hint_operation_panel.getContentSize().height/2));
				}
			}
			let num = i;
			bright_tiles_btn.setScale(1.1);
			bright_tiles_btn.addTouchEventListener(function (sender, eventType) {
				if (eventType == ccui.Widget.TOUCH_ENDED) {
					if (num === 0) {num = 1;}
					if (windsTileNum === 3 && kingNum >= 1) {num = kingNum;}
					if (windsTileNum === kingNum) {num = kingNum;}
					if (windsTileNum - kingNum >= 3) {num = 0;}
					h1global.player().hintDTLGFMJ(num);
					h1global.curUIMgr.roomLayoutMgr.notifyObserver("show_hint_operation_panel", false);
		...
		hint_operation_panel.getChildByName("pass_bright_tiles_btn").addTouchEventListener(function (sender, eventType) {
			if (eventType == ccui.Widget.TOUCH_ENDED) {
				h1global.player().hintDTLGFMJ(-1);
				h1global.curUIMgr.roomLayoutMgr.notifyObserver("show_hint_operation_panel", false);
				if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow() && !isWin) {
					h1global.curUIMgr.roomLayoutMgr.notifyObserver("unlock_player_hand_tiles");
					h1global.curUIMgr.roomLayoutMgr.notifyObserver("update_operation_panel", player.gameOperationAdapter.getDrawOpDict(player.curGameRoom.lastDrawTile), const_val.SHOW_DO_OP);
				} else {
					h1global.curUIMgr.roomLayoutMgr.notifyObserver("unlock_player_hand_tiles");
					h1global.curUIMgr.roomLayoutMgr.notifyObserver("hide_operation_panel");
				}
	...
	update_show_lucky_tiles_panel:function () {
		if (!this.is_show) {return;}
		var liangpai_json = ccs.load("res/ui/liangpai.json");   //加载CocosStudio导出的Json文件
		var liangpai_node = liangpai_json.node;
		var liangpai_action = liangpai_json.action; // 动作
		liangpai_action.gotoFrameAndPlay(0,liangpai_action.getDuration(),0,false);
		liangpai_node.runAction(liangpai_action); // 播放动作
		this.rootUINode.addChild(liangpai_node);  //将UI输出到画布
		liangpai_node.setPosition(cc.p(cc.winSize.width * 0.5, cc.winSize.height * 0.5));
		liangpai_node.setScale(2);
	},

	show_lucky_tiles_panel:function (visible) {
		this.rootUINode.getChildByName("show_lucky_tiles_panel").setVisible(visible);
	},
