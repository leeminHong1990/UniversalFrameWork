...
//
"use strict"
var DTLGFMJPlaybackGameRoom = cc.Node.extend({
    ...
    init: function () {
	    ...
	    this.roomUI.update_hint_operation_panel = function () {

        };
	    this.roomUI.show_hint_operation_panel = function () {

	    };
        this.roomUI.rootUINode.getChildByName("hint_operation_panel").setVisible(false);
        this.roomUI.update_playback_operation_panel = function (serverSitNum, op_dict, doOP) {
            if(doOP == const_val.OP_DRAW && h1global.curUIMgr.playback_ui){
                h1global.curUIMgr.playback_ui.hideOperationPanel()
            }
    ...
    doNext: function () {
        ...
        if (commands.length > 0) {
            ...
            if (command.aid == const_dtlgfmj.OP_SHOW_HINT){
                player.postHintOperationDTLGFMJ(command.serverSitNum, command.tileList[1], command.tileList[0]);
            } else if(command.aid == const_dtlgfmj.OP_ADD_HINT){
                player.postAddHintDTLGFMJ(command.serverSitNum, command.tileList);
            } else {
        ...
        if (op_special_record) {
            ...
            while (flag) {
                ...
                if (re[3] == this.totalCommand - this.commands.length) {
                    ...
                    // player.postPlayerDiscardState(re[1], const_dtlgfmj.DISCARD_FORCE);
