...
var DTLGFMJSettlementUI = SettlementUI.extend({
    ...
    show_by_info: function (roundRoomInfo, serverSitNum, curGameRoom, confirm_btn_func, replay_btn_func) {
		...
		this.show(function(){
			...
			for(var i = 0; i < 4; i++){
                ...
                self.update_player_win(i, roundRoomInfo["win_idx"], roundRoomInfo["from_idx"], roundRoomInfo["dealer_idx"], roundRoomInfo["result_list"], roundRoomInfo["serial_dealer"], roundRoomInfo["hint_tiles"]);
    ...
    show_rules : function (curGameRoom) {
        ...
        if(curGameRoom.game_mode === 1){
            share_list.push("自摸胡");
        }

        if(curGameRoom.score_mode === 1){
            share_list.push("防作弊玩法");
        }

        if(curGameRoom.kong_mode === 1){
            share_list.push("杠算分");
        }

        if(curGameRoom.seven_pair === 1){
            share_list.push("七对");
        }

        if(curGameRoom.base_score){
            share_list.push("底分" + curGameRoom.base_score);
        }
	...
	update_player_hand_tiles:function(serverSitNum, curGameRoom ,tileList, win_idx, finalTile, concealedKongList){
		...
		if(win_idx == serverSitNum) {
            tileList.pop();
            tileList = tileList.sort(cutil_dtlgfmj.tileSortFunc);
	...
	update_player_win:function(serverSitNum, win_idx, from_idx, dealer_idx, result, serial_dealer, hint_tiles){
		var cur_player_info_panel = this.player_tiles_panels[serverSitNum];
        ...
        if(serverSitNum == win_idx) {
            for (var i = 0; i < result.length; i++) {
                if (result[i]) {
                    item_win_type_label.string += const_dtlgfmj.WIN_TYPE_LIST[i] + "   ";
                }
            }

            if(serial_dealer > 0){
                item_win_type_label.string += serial_dealer + "连庄   ";
			}

			var str_eswn = ["东","南","西","北"];
			for(var i=0; i<const_val.WINDS.length; i++){
				var count = cutil_dtlgfmj.get_count(hint_tiles, const_val.WINDS[i]);
				if(count > 0){
                    item_win_type_label.string += str_eswn[i] + "x" + count + " ";
				}
			}

            var str_zfb = ["中","发","白"];
            for(var i=0; i<const_val.DRAGONS.length; i++){
                var count = cutil_dtlgfmj.get_count(hint_tiles, const_val.DRAGONS[i]);
                if(count > 0){
                    item_win_type_label.string += str_zfb[i] + "x" + count + " ";
                }
            }
        }
