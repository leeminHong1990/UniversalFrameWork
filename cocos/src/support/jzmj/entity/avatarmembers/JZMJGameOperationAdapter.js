...
/*-----------------------------------------------------------------------------------------
												interface
-----------------------------------------------------------------------------------------*/
var JZMJGameOperationAdapter = JZMJPlaybackOperationAdapter.extend({
	...
	startGame : function(startInfo){
        ...
        if (this.serverSitNum == dealerIdx) {
			...
			cutil_jzmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], kingTiles);
    ...
    removeHandTile: function (serverSitNum, tileNum, count , sort) {
        ...
        if(sort){
            cutil_jzmj.tileSort(this.curGameRoom.handTilesList[serverSitNum], this.curGameRoom.kingTiles)
        }
    },
		...
		if(aid == const_val.OP_PASS){
			...
			if(this.serverSitNum == serverSitNum){
				...
				if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
					...
					//轮到自己摸牌，是否可以随意出牌
					if (this.curGameRoom.discardStateList[this.serverSitNum] === const_jzmj.DISCARD_FORCE) {
			...
			if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
				...
				h1global.curUIMgr.roomLayoutMgr.notifyObserver("update_canwin_tile_panel", const_jzmj.NOT_DISPLAY_CANWIN_PANEL);
			}
			// if(this.curGameRoom.kingTiles.indexOf(tileList[0]) >= 0){
			if(buckle === 1) {
				if(this.curGameRoom.playerInfoList[serverSitNum]["sex"] == 1){
			...
			if(this.serverSitNum == serverSitNum){
                ...
                cutil_jzmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], this.curGameRoom.kingTiles);
                var sortChowTiles = cutil_jzmj.sortChowTileList(tileList[0], tileList.concat());
                // cutil_jzmj.tileSort(sortChowTiles, this.curGameRoom.kingTiles);
			...
			if(this.serverSitNum == serverSitNum){
				...
				var numStand =  cutil_jzmj.get_count(standTileList, tileList[0]);
			...
			if(this.serverSitNum == serverSitNum){
                ...
                var standTileList = this.curGameRoom.standTileList[serverSitNum];
                var numStand =  cutil_jzmj.get_count(standTileList, tileList[3]);
	...
	doOperation : function(aid, tileList){
		...
		if(this.curGameRoom.curPlayerSitNum === this.serverSitNum && aid === const_val.OP_PASS){
			...
			if (this.checkPassWin) {
				h1global.player().setPassWinStateJZMJ(const_jzmj.PASS_WIN, tileList[0]);
				this.checkPassWin = false;
			}
			// return;
		}
		...
		if (this.curGameRoom.discardStateList[this.serverSitNum] === const_jzmj.DISCARD_FREE) {
	...
	waitForOperation : function(aid_list, tileList){
		...
		// 摸打模式
		if ((aid_list.indexOf(const_val.OP_DRAW_WIN) >= 0 || aid_list.indexOf(const_val.OP_KONG_WIN) >= 0 || aid_list.indexOf(const_val.OP_GIVE_WIN) >= 0) && this.curGameRoom.discardStateList[this.serverSitNum] === const_jzmj.DISCARD_FORCE) {
	...
	// setPassWinStateJZMJ:function (state) {
	// 	if(!this.curGameRoom){
	// 		return;
	// 	}
	// 	cc.warn("setPassWinStateJZMJ state:",state);
	// 	this.curGameRoom.pass_win_list[this.serverSitNum] = state;
	// 	this.sourcePlayer.cellCall("setPassWinStateJZMJ", state);
	// },
