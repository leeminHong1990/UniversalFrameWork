...
/*-----------------------------------------------------------------------------------------
 interface
 -----------------------------------------------------------------------------------------*/
var JZMJGameRules = JZMJRoomOperationAdapter.extend({
    ...
    getDrawOpDict: function (drawTile, serverSitNum) {
        ...
        if (this.isOpLimit()) {
            //胡
            if (handTiles.length % 3 === 2 && this.canWin(handTiles, drawTile, serverSitNum, const_val.OP_DRAW_WIN) && this.curGameRoom.discardStateList[serverSitNum] === const_jzmj.DISCARD_FORCE) {
        ...
        for (var i = 0; i < handTiles.length; i++) {
            for (var j = 0; j < uptiles.length; j++) {
                ...
                if (upMeld.length === 3 && upMeld[0] === upMeld[1] && upMeld[1] === upMeld[2] && upMeld[0] === handTiles[i] && this.curGameRoom.discardStateList[this.serverSitNum] !== const_jzmj.DISCARD_FORCE) {
        ...
        //暗杠
        var tile2NumDict = cutil_jzmj.getTileNumDict(handTiles)
        for (var key in tile2NumDict) {
            // if (this.curGameRoom.kingTiles.indexOf(eval(key)) >= 0 && this.curGameRoom.reward === 1){
            //     continue;
            // }
            if (tile2NumDict[key] === 4 && this.curGameRoom.discardStateList[this.serverSitNum] !== const_jzmj.DISCARD_FORCE) {
                ...
                if (standTileList.indexOf(parseInt(key)) >= 0 && cutil_jzmj.get_count(standTileList, parseInt(key)) === standTileList.length) {
	                continue;
                }
	    ...
	    if (this.curGameRoom.discardStateList[this.serverSitNum] === const_jzmj.DISCARD_FORCE && this.curGameRoom.canWinCanSelectList[this.serverSitNum] == 0) {
		    op_dict = {};
        }
        return op_dict
    },
    ...
    nomalCanWin: function (handTiles, finalTile, serverSitNum, aid) {
        ...
        var kingClassified = cutil_jzmj.classifyKingTiles(handCopyTile, this.curGameRoom.kingTiles);
        var kings   = kingClassified[0];
        var handTilesButKing  = kingClassified[1];
        var kingTilesNum = kings.length;
	    var colorType = cutil_jzmj.getTileColorType(handTiles, uptiles, this.curGameRoom.kingTiles);
	    if (colorType === const_val.MIX_X_SUIT) {
		    return false;
	    }
	    if (cutil_jzmj.checkIs7Pairs(handTilesButKing, kingTilesNum)) {              // 7对
            return true;
        } else if(cutil_jzmj.canNormalWinWithoutKing3N2(handTilesButKing)) {        // 没癞子
		    return true;
        }
    },
