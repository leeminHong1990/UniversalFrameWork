...
var JZMJGameRoomEntity = GameRoomEntity.extend({
	ctor: function (player_num, gameType) {
		...
		this.standTileList = [[], [], [], []];
		this.canWinCanSelectList = [0,0,0,0];

		this.discardStateList = [const_jzmj.DISCARD_FREE, const_jzmj.DISCARD_FREE, const_jzmj.DISCARD_FREE, const_jzmj.DISCARD_FREE]
		...
		this.pass_win_list = [const_jzmj.NO_PASS_WIN,const_jzmj.NO_PASS_WIN,const_jzmj.NO_PASS_WIN,const_jzmj.NO_PASS_WIN]; //玩家是否过胡
  	...
  	reconnectRoomData : function(recRoomInfo){
	    ...
	    this.canWinCanSelectList = recRoomInfo["canWinCanSelectList"];
  		for(var i = 0; i < recRoomInfo["player_advance_info_list"].length; i++){
  			...
  			for(var j = 0; j < curPlayerInfo["op_list"].length; j++){
  				...
  				if(op_info["opId"] === const_val.OP_PONG){
  					...
  					sortTiles = cutil_jzmj.sortChowTileList(sortTiles[0], sortTiles);
                    // cutil_jzmj.tileSort(sortTiles, this.kingTiles);
  	...
  	updateRoomData : function(roomInfo){
	    ...
	    this.stand_four = roomInfo["stand_four"];
