...
"use strict"
var TYKDDMJGameRoomUI = UIBase.extend({
	ctor:function() {
		this._super();
		this.talk_img_num = 0;
	},
    initUI:function(){
        ...
        this.touch_state = const_tykddmj.PLAYER_TOUCH_SELF_STATE
    ...
    init_ting_panel:function(){
        ...
        function ting_btn_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
                ...
                // player.setDiscardState(const_tykddmj.DISCARD_FORCE);
                // self.checkTing = true;
	            h1global.curUIMgr.roomLayoutMgr.iterUI(function (ui) {
		            ui.checkTing = true;
	            });
        ...
        function guo_btn_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
                var player = h1global.entityManager.player();
                player.setDiscardState(const_tykddmj.DISCARD_FREE);
	            h1global.curUIMgr.roomLayoutMgr.iterUI(function (ui) {
		            ui.checkTing = false;
                });
                // self.checkTing = false;
    ...
    force_discard:function () {
        ...
        // return;
        // var player = h1global.player();
        // this.touch_state = const_tykddmj.PLAYER_TOUCH_FORCE_STATE;
    ...
    update_player_info_panel:function(serverSitNum, playerInfo){
        ...
        if (dealer_img && cur_player_info_panel) {
    ...
    _resetSelectTile: function () {
        ...
        if (this.curSelectTile) {
            ...
            this.update_canwin_tile_panel(const_tykddmj.NOT_DISPLAY_CANWIN_PANEL);
        }
    },
    ...
    init_player_tile_panel:function(){
        ...
        function unchoose_tile() {
            ...
            self.update_canwin_tile_panel(const_tykddmj.NOT_DISPLAY_CANWIN_PANEL);
        }
        ...
        function discard_tile(tile, discard_pos) {
            ...
            if (player){
                ...
                if (self.checkTing && player.gameOperationAdapter.canTingTiles.indexOf(tile.tileNum) >= 0) {
                    player.setDiscardState(const_tykddmj.DISCARD_FORCE, tile.tileNum);
        ...
        function hand_tile_event(tile, touch_mode, sender){
            ...
            if (self.touch_state === const_tykddmj.PLAYER_TOUCH_SELF_STATE && !player.gameOperationAdapter.canDiscardIdx(tile.index)){ //别人打财神后 限制出牌
                return
            }
            if (self.checkTing && player.gameOperationAdapter.canTingTiles.indexOf(tile.tileNum) < 0) {
                return;
            ...
            if(touch_mode == ccui.Widget.TOUCH_BEGAN){ // 只选中
                ...
                if(self.choose_tile_list.length == 2 && self.choose_tile_list[0].index == self.choose_tile_list[1].index && self.curSelectTile.index == self.choose_tile_list[0].index){
                    ...
                    if(self.touch_state !== const_tykddmj.PLAYER_TOUCH_SELF_STATE){ // 非出牌状态不能出牌
                        return
                    }
                    discard_tile(tile)
        ...
        function player_hand_panel_event(sender, eventType){
            // NOTE:下乡禁止触摸和滑动
            if(self.touch_state === const_tykddmj.PLAYER_TOUCH_FORCE_STATE || player.curGameRoom.discardStateList[player.serverSitNum] === const_tykddmj.DISCARD_FORCE){
                if(self.curSelectTile){
                    reset_tile()
                }
                return;
            ...
            if (eventType == ccui.Widget.TOUCH_BEGAN) {
	            if (player.gameOperationAdapter.canTingTiles) {
		            if (player.gameOperationAdapter.canTingTiles.length > 0) {
			            if (self.curSelectTile && player.gameOperationAdapter.canTingTiles.indexOf(self.curSelectTile.tileNum) < 0 && player.curGameRoom.discardStateList[player.serverSitNum] === const_tykddmj.DISCARD_FORCE) {
				            reset_tile()
				            return;
			            }
		            }
                ...
                touch_func(p, eventType, sender)
                if (self.curSelectTile && !self.moving_tile && self.touch_state === const_tykddmj.PLAYER_TOUCH_SELF_STATE && p.y - self.curSelectTile.getPositionY() > 76) { // 向上拖 生成一个 麻将子
                ...
                if (self.moving_tile) {
                    ...
                    // 出牌
                    if (py > hand_panel_height + config.sel_posy && self.touch_state === const_tykddmj.PLAYER_TOUCH_SELF_STATE) {
    ...
    mark_discard_king_tiles:function () {
        ...
        for(var i = 0; i < list.length; i++){
            ...
            let start = 0 ;
            if(tileSum > const_tykddmj.MAX_DISCARD_TILES_SIZE){
                start = tileSum - const_tykddmj.MAX_DISCARD_TILES_SIZE;
            }
    ...
    mark_same_tiles:function(tileNum){
        ...
        for(var i = 0; i < list.length; i++){
            ...
            var maxDeskSize = const_tykddmj.MAX_DISCARD_TILES_SIZE;
            if (player.curGameRoom.discardStateList[i] === const_tykddmj.DISCARD_FORCE) {
    ...
    cancel_mark_same_tiles:function(){
        ...
        for(var i = 0; i < list.length; i++){
            ...
            for(var j = start; j< tileSum; j++){
                if(player.curGameRoom.kingTiles.indexOf(tiles[j]) < 0)	{
                    var tile_img = cur_player_discard_panel.getChildByName("tile_img_" + (j  - start));
                    if (tile_img.color.b === const_tykddmj.mark_ting_color.b) {
                        continue;
                    }
                    this.cancel_tile_color(tile_img);
                }
    ...
    lock_player_hand_tiles:function(){
        if(!this.is_show) {return;}
        var player = h1global.player();
        this.touch_state = const_tykddmj.PLAYER_TOUCH_OTHER_STATE;
    ...
    update_player_hand_tiles: function (serverSitNum, tileList) {
        ...
        for (var i = 0; i < 14; i++) {
            ...
            if(handTilesList[i] !== undefined){
                if(handTilesList[i] > 0){
                    ...
                    if(isPlayer){
                        if(discardState === const_tykddmj.DISCARD_FORCE){
                            this.mark_same_tile(tile_img);
                            this.lock_player_hand_tiles();
                        }else{
                            this.cancel_tile_color(tile_img);
        ...
        if(serverSitNum == player.serverSitNum && player.curGameRoom.discardStateList[serverSitNum] === const_tykddmj.DISCARD_FREE){
    ...
    play_discard_anim:function(serverSitNum, onlyTilePointer, buckle){
        ...
        if(discardTilesSum > const_tykddmj.MAX_DISCARD_TILES_SIZE) discardIndex = const_tykddmj.MAX_DISCARD_TILES_SIZE - 1;
    ...
    update_player_discard_tiles:function(serverSitNum){
        ...
        var deskSize = const_tykddmj.DISCARD_TILES_SIZE;
        var maxDeskSize = const_tykddmj.MAX_DISCARD_TILES_SIZE;
        ...
        var discardTilesList = player.curGameRoom.discardTilesList[serverSitNum];
        var tingTilePos = undefined;
        if (player.curGameRoom.discardStateList[serverSitNum] === const_tykddmj.DISCARD_FORCE) {
        ...
        for(var i = 0; i < deskSize; i++){
            ...
            if(i < maxDeskSize && discardTilesList[i] > 0){
                ...
                if (tingTilePos >= 0 && tingTilePos === i) {
                    if (isPlayer) {
                        // this.mark_same_tile(tile_img);
                        tile_img.color = const_tykddmj.mark_ting_color;
                    } else {
                        if (options.mahjongdesk) {
    ...
    update_wintips_btn:function () {
        ...
        if(canWinTiles.length <= 0 || player.curGameRoom.curPlayerSitNum == player.serverSitNum){
            ...
            wintips_btn.addTouchEventListener(function (sender, eventType) {
                if (eventType === ccui.Widget.TOUCH_BEGAN) {
                    self.update_canwin_tile_panel(const_tykddmj.WINTIPS_BTN_DISPLAY);
    ...
    update_canwin_tile_panel:function(select_tile){
        ...
        var self = this;
        var canwin_tile_panel = self.rootUINode.getChildByName("canwin_tile_panel");
        if(select_tile == const_tykddmj.NOT_DISPLAY_CANWIN_PANEL){
            canwin_tile_panel.setVisible(false);
            return;
        }
        var player = h1global.player();
    ...
    play_result_anim: function (player_info_list) {
        ...
        for (var i = 0; i < curGameRoom.player_num; i++) {
            ...
            cutil_tykddmj.tileSort(handTiles, player.curGameRoom.kingTiles);
    ...
    update_operation_panel:function(op_dict, from_type){
        ...
        var self = this;
        var player = h1global.player();
        // if (player.curGameRoom.discardStateList[player.serverSitNum] === const_tykddmj.DISCARD_FORCE && player.curGameRoom.game_mode !== const_tykddmj.KING_GAME_MODE) {
