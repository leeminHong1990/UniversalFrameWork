"use strict";
var TYKDDMJGameOperationAdapter = TYKDDMJPlaybackOperationAdapter.extend({
    ...
    startGame : function(startInfo){
        ...
        if (this.serverSitNum == dealerIdx) {
            ...
            cutil_tykddmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], kingTiles);
    ...
    removeHandTile: function (serverSitNum, tileNum, count , sort) {
        ...
        if(sort){
            cutil_tykddmj.tileSort(this.curGameRoom.handTilesList[serverSitNum], this.curGameRoom.kingTiles)
        }
    },
    ...
    postOperation : function(serverSitNum, aid, tileList, buckle){
        ...
        if(aid == const_val.OP_PASS){
            ...
            if (h1global.curUIMgr && h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
                ...
                h1global.curUIMgr.roomLayoutMgr.notifyObserver("update_canwin_tile_panel", const_tykddmj.NOT_DISPLAY_CANWIN_PANEL);
            }
            // if(this.curGameRoom.kingTiles.indexOf(tileList[0]) >= 0){
            if(buckle === 1) {
            ...
            if(this.serverSitNum == serverSitNum){
                ...
                cutil_tykddmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], this.curGameRoom.kingTiles);
                var sortChowTiles = cutil.sortChowTileList(tileList[0], tileList.concat());
                // cutil_tykddmj.tileSort(sortChowTiles, this.curGameRoom.kingTiles);
    ...
    doOperation : function(aid, tileList){
        ...
        if(this.curGameRoom.curPlayerSitNum === this.serverSitNum && aid === const_val.OP_PASS){
            ...
            if (this.checkPassWin) {
                this.setPassWinState(const_tykddmj.PASS_WIN);
                this.checkPassWin = false;
            }
            // return;
        }
    ...
    waitForOperation : function(aid_list, tileList){
        ...
        // 摸打模式
        if ((aid_list.indexOf(const_val.OP_DRAW_WIN) >= 0 || aid_list.indexOf(const_val.OP_KONG_WIN) >= 0 || aid_list.indexOf(const_val.OP_GIVE_WIN) >= 0) && this.curGameRoom.discardStateList[this.serverSitNum] === const_tykddmj.DISCARD_FORCE) {
