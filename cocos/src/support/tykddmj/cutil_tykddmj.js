"use strict";
var cutil_tykddmj = function(){};

cutil_tykddmj.get_count = function(tiles, t){
    var sum = 0;
...
cutil_tykddmj.meld_history = {};

cutil_tykddmj.meld_with_pair_need_num = function(tiles, history, used) {
    history = history || this.meld_history;
    var case1 = 999;
    var case2 = 999;
    var idx = -1;

    if (cutil_tykddmj.meld_only_need_num(tiles, history) === 0){
        case1 = 2;
    }
    ...
    for(var i = 0; i < tiles.length; i++){
        ...
        if (cutil_tykddmj.get_count(tiles, tiles[i]) === 1){
            idx = tmp.indexOf(tiles[i]);
            tmp.splice(idx, 1);
            case2 = Math.min(case2, 1 + cutil_tykddmj.meld_only_need_num(tmp, history));
        } else {
            idx = tmp.indexOf(tiles[i]);
            tmp.splice(idx, 1);
            idx = tmp.indexOf(tiles[i]);
            tmp.splice(idx, 1);
            case2 = Math.min(case2, cutil_tykddmj.meld_only_need_num(tmp, history));
        }
    }
...
cutil_tykddmj.meld_only_need_num = function(tiles, history, used){
    history = history || this.meld_history;
    used = used || 0;
    if (used > 4){
        return 999;
...
cutil_tykddmj.checkIs7Pairs = function(handTilesButKing, kingTilesNum){
    if (handTilesButKing.length + kingTilesNum !== 14) {return false}
    var tileDict = cutil_tykddmj.getTileNumDict(handTilesButKing);
    var need_num = 0
...
// 十三幺
cutil_tykddmj.getThirteenOrphans = function (handTilesButKing, kingTilesNum) {
    ...
    function withoutKings() {
        ...
        for (var i = 0; i < need_tiles.length; i++) {
            if (cutil_tykddmj.get_count(handTilesButKing, need_tiles[i]) === 2) {
                return true;
            }
        }
        return false;
...
cutil_tykddmj.classifyTiles = function(tiles, kingTiles){
    kingTiles = kingTiles || [];
    var kings = [];
    var chars = [];
    var bambs = [];
...
cutil_tykddmj.checkIs6Pairs = function(tilesButKing, kingTilesNum){
    if (tilesButKing.length + kingTilesNum !== 12) {return false}
    var tileDict = cutil_tykddmj.getTileNumDict(tilesButKing);
    var need_num = 0
    for(var tile in tileDict){
        need_num += (tileDict[tile] % 2);
    }
...
cutil_tykddmj.canNormalWinWithKing3N2 = function (handTilesButKing, kingTilesNum) {
    if ((handTilesButKing.length + kingTilesNum) % 3 !== 2) {
        return false;
    }
    var classified = cutil_tykddmj.classifyTiles(handTilesButKing, []);
...
cutil_tykddmj.canNormalWinWithKing3N = function (handTilesButKing, kingTilesNum) {
...
cutil_tykddmj.canNormalWinWithoutKing3N2 = function (handTiles) {
    if (handTiles.length % 3 !== 2) {
        return false;
    }
    var classified = cutil_tykddmj.classifyTiles(handTiles);
    var chars	= classified[1];
    var bambs	= classified[2];
    var dots	= classified[3];
    var winds	= classified[4];
...
cutil_tykddmj.isMeld = function (tiles) {
    if (tiles.length % 3 !== 0) {
        return false;
    }
...
cutil_tykddmj.isMeldWithPair = function (tiles) {
    if (tiles.length % 3 !== 2) {
        return false;
    }
    var total = 0;
...
cutil_tykddmj.checkMeldInPossible = function (tiles, possibleList) {
    var idx;
...
cutil_tykddmj.sortChowTileList = function (chowTile, tiles) {
...
cutil_tykddmj.tileSort = function (tiles, kingTiles) {
    var kings = kingTiles || [];
...
cutil_tykddmj.classifyKingTiles = function(tiles, kingTiles){
    kingTiles = kingTiles || [];
    var kings = [];
    var others = [];
    for (var i = 0; i < tiles.length; i++) {
...
//获取同样牌的张数 dict
cutil_tykddmj.getTileNumDict = function(tiles){
    var tileDict = {}
    for (var i = 0; i < tiles.length; i++) {
        var t = tiles[i]
        if (!tileDict[t]) {
...
cutil_tykddmj.count = function(list, key){
    var dict = {}
...
cutil_tykddmj.tileSortFunc = function(a, b){
	if(a == b){
		return 0;
	}
...
cutil_tykddmj.getMeldNeed = function (handTiles) {
    var classified = cutil_tykddmj.classifyTiles(handTiles);
	var chars	= classified[1];
	var bambs	= classified[2];
	var dots	= classified[3];
	...
	for (var i = 0 ; i < tileList.length ; i++) {
	    var num = cutil_tykddmj.meld_only_need_num(tileList[i]);
	    sum += num;
    }
    return sum;
};
...
//耗子吊将牌型判断
cutil_tykddmj.isMouseGeneral = function (handTiles, handTilesButKing, kingTilesNum, kingTiles, finalTile) {
	...
	var needKingNum = cutil_tykddmj.getMeldNeed(handCopyTilesButKing);
