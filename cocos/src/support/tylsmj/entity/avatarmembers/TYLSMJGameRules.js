...
/*-----------------------------------------------------------------------------------------
 interface
 -----------------------------------------------------------------------------------------*/
var TYLSMJGameRules = TYLSMJRoomOperationAdapter.extend({
    ...
    getDrawOpDict: function (drawTile, serverSitNum) {
        ...
        if (this.isOpLimit()) {
            //胡
            if (handTiles.length % 3 === 2 && this.canWin(handTiles, drawTile, serverSitNum, const_val.OP_DRAW_WIN) && this.curGameRoom.discardStateList[serverSitNum] === const_tylsmj.DISCARD_FORCE) {
        ...
        //暗杠
        var tile2NumDict = cutil_tylsmj.getTileNumDict(handTiles)
        for (var key in tile2NumDict) {
            ...
            if (tile2NumDict[key] === 4) {
                ...
                if (standTileList.indexOf(parseInt(key)) >= 0 && cutil_tylsmj.get_count(standTileList, parseInt(key)) === standTileList.length) {
	                continue;
                }
	    ...
	    // if (this.curGameRoom.discardStateList[this.serverSitNum] === const_tylsmj.DISCARD_FORCE && this.curGameRoom.game_mode === const_val.KING_GAME_MODE) {
    ...
    nomalCanWin: function (handTiles, finalTile, serverSitNum, aid) {
        ...
        var kingClassified = cutil_tylsmj.classifyKingTiles(handCopyTile, this.curGameRoom.kingTiles);
        ...
        if(kingTilesNum > 0){                                            // 有癞子
            return cutil_tylsmj.canNormalWinWithKing3N2(handTilesButKing, kingTilesNum);
        } else {                                                                // 没癞子
            return cutil_tylsmj.canNormalWinWithoutKing3N2(handTilesButKing);
        }
    },
