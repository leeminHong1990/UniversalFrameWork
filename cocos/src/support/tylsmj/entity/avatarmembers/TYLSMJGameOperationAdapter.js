...
/*-----------------------------------------------------------------------------------------
												interface
-----------------------------------------------------------------------------------------*/
var TYLSMJGameOperationAdapter = TYLSMJPlaybackOperationAdapter.extend({

	// startGame : function(dealerIdx, tileList, wreathsList, kingTiles, prevailing_wind, playerWindList, diceList, swap_list, standTileList){
	startGame : function(startInfo){
		...
		var standTileList = startInfo['standTileList'];
		//交换位置 玩家当前在服务端的位置也改变
		...
		this.curGameRoom.standTileList = standTileList;
        ...
        if (this.serverSitNum == dealerIdx) {
			...
			cutil_tylsmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], kingTiles);
        ...
        if(this.curGameRoom.curRound <= 1) {
            this.sourcePlayer.startActions["GameRoomScene"] = function(){
	            ...
	            if (h1global.curUIMgr.tylsmjgameroominfo_ui) {
		            if(h1global.curUIMgr.tylsmjgameroominfo_ui.is_show){
			            h1global.curUIMgr.tylsmjgameroominfo_ui.hide();
		            }
		...
		if(h1global.curUIMgr.tylsmjgameroominfo_ui && h1global.curUIMgr.tylsmjgameroominfo_ui.is_show){
			h1global.curUIMgr.tylsmjgameroominfo_ui.update_round();
			h1global.curUIMgr.tylsmjgameroominfo_ui.update_round_wind(prevailing_wind);
		}
    ...
    removeHandTile: function (serverSitNum, tileNum, count , sort) {
        ...
        if(sort){
            cutil_tylsmj.tileSort(this.curGameRoom.handTilesList[serverSitNum], this.curGameRoom.kingTiles)
        }
    },
	
	postOperation : function(serverSitNum, aid, tileList, buckle, standTileList){
		cc.log("postOperation: ", serverSitNum, aid, tileList, buckle, standTileList);
		if(!this.curGameRoom){
			return;
		}
		buckle = buckle || 0;
		this.curGameRoom.standTileList = standTileList || this.curGameRoom.standTileList;
		...
		if(aid == const_val.OP_PASS){
			...
			if(this.serverSitNum == serverSitNum){
				...
				if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
					...
					//轮到自己摸牌，是否可以随意出牌
					if (this.curGameRoom.discardStateList[this.serverSitNum] === const_tylsmj.DISCARD_FORCE) {
			...
			if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
				...
				h1global.curUIMgr.roomLayoutMgr.notifyObserver("update_canwin_tile_panel", const_tylsmj.NOT_DISPLAY_CANWIN_PANEL);
			}
			// if(this.curGameRoom.kingTiles.indexOf(tileList[0]) >= 0){
			if(buckle === 1) {
				if(this.curGameRoom.playerInfoList[serverSitNum]["sex"] == 1){
			...
			if(this.serverSitNum == serverSitNum){
                ...
                cutil_tylsmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], this.curGameRoom.kingTiles);
                var sortChowTiles = cutil_tylsmj.sortChowTileList(tileList[0], tileList.concat());
                // cutil_tylsmj.tileSort(sortChowTiles, this.curGameRoom.kingTiles);
			...
			if(this.serverSitNum == serverSitNum){
				...
				var standTileList = this.curGameRoom.standTileList[serverSitNum];
				var numStand =  cutil_tylsmj.get_count(standTileList, tileList[0]);
				if (standTileList.indexOf(tileList[0]) >= 0) {
					standTileList.splice(standTileList.indexOf(tileList[0]), numStand);
				}
                this.removeHandTile(this.serverSitNum, tileList[0], 1, true);
			...
			if(this.serverSitNum == serverSitNum){
                ...
                var standTileList = this.curGameRoom.standTileList[serverSitNum];
                var numStand =  cutil_tylsmj.get_count(standTileList, tileList[3]);
                if (standTileList.indexOf(tileList[3]) >= 0) {
	                standTileList.splice(standTileList.indexOf(tileList[3]), numStand);
                }
	...
	doOperation : function(aid, tileList){
		...
		if(this.curGameRoom.curPlayerSitNum === this.serverSitNum && aid === const_val.OP_PASS){
			...
			if (this.checkPassWin) {
				// this.setPassWinState(const_tylsmj.PASS_WIN);
				this.checkPassWin = false;
			}
			// return;
		}
		...
		if (this.curGameRoom.discardStateList[this.serverSitNum] === const_tylsmj.DISCARD_FREE) {
	...
	waitForOperation : function(aid_list, tileList){
		...
		// 摸打模式
		// if ((aid_list.indexOf(const_val.OP_DRAW_WIN) >= 0 || aid_list.indexOf(const_val.OP_KONG_WIN) >= 0 || aid_list.indexOf(const_val.OP_GIVE_WIN) >= 0) && this.curGameRoom.discardStateList[this.serverSitNum] === const_tylsmj.DISCARD_FORCE && this.curGameRoom.game_mode === const_val.KING_GAME_MODE) {
	...
	setDiscardState : function(state, tile){
		...
		cc.log("setDiscardState:",state, tile);
		this.sourcePlayer.cellCall("setDiscardState", state, tile);
	},
	...
	postWinOperation:function (serverSitNum, op, result) {
        ...
        // 播放 胡牌结果语音
		var value = -1; // 1 自摸 2 平胡 3 爆头 4 杠暴 5 杠开 6 豪七 7 豪七暴 8 七暴 9 七对子 10 清七对
		var voice_list = ["draw_win", "win", "baotou", "gangbao", "gangkai", "haoqi", "haoqibao", "qibao", "qiduizi", "qingqidui"];
