...
"use strict"
var TYLSMJGameRoomUI = UIBase.extend({
	ctor:function() {
		this._super();
		this.talk_img_num = 0;
	},
	initUI:function(){
		...
		if (h1global.curUIMgr.tylsmjgameroominfo_ui && !h1global.curUIMgr.tylsmjgameroominfo_ui.is_show) {
			h1global.curUIMgr.tylsmjgameroominfo_ui.show_by_info();
		}

		if (h1global.curUIMgr.gameroominfo_ui) {
			if (h1global.curUIMgr.tylsmjgameroominfo_ui) {
				h1global.curUIMgr.gameroominfo_ui.hide();
			} else if (!h1global.curUIMgr.tylsmjgameroominfo_ui){
	...
	init_ting_panel:function(){
		...
		function ting_btn_event(sender, eventType){
			if (eventType === ccui.Widget.TOUCH_ENDED) {
				...
				// player.setDiscardState(const_tylsmj.DISCARD_FORCE);
				h1global.curUIMgr.roomLayoutMgr.iterUI(function (ui) {
					ui.checkTing = true;
				});
		...
		function guo_btn_event(sender, eventType){
			if (eventType === ccui.Widget.TOUCH_ENDED) {
				var player = h1global.entityManager.player();
				player.setDiscardState(const_tylsmj.DISCARD_FREE);
				h1global.curUIMgr.roomLayoutMgr.iterUI(function (ui) {
					ui.checkTing = false;
				});
	...
	_setTileMask:function () {
		...
		for( var i = 0 ; i < handTiles.length ; i++) {
			...
			if ((i < temp_standTileList.length || this.standTileIdxList.indexOf(i) >= 0) && temp_standTileList.indexOf(handTiles[i]) >= 0 && index >= 0) {
				...
				tile_img.color = const_tylsmj.mark_same_color;
			}
		}
	},
    ...
    _resetSelectTile: function () {
        ...
        if (this.curSelectTile) {
            ...
            this.update_canwin_tile_panel(const_tylsmj.NOT_DISPLAY_CANWIN_PANEL);
        }
    },
	...
	init_player_tile_panel:function(){
        ...
        function unchoose_tile() {
            ...
            self.update_canwin_tile_panel(const_tylsmj.NOT_DISPLAY_CANWIN_PANEL);
        }
        ...
        function discard_tile(tile, discard_pos) {
            ...
            if (player){
	            ...
	            if (self.checkTing && player.gameOperationAdapter.canTingTiles.indexOf(tile.tileNum) >= 0) {
		            player.setDiscardState(const_tylsmj.DISCARD_FORCE, tile.tileNum);
		...
		function touch_func(touch_pos, eventType, sender){
			...
			if (self.isArrange) {
				...
				if (player.curGameRoom.standTileList[player.serverSitNum].length == player.curGameRoom.handTilesList[player.serverSitNum].length) {
					//这里特殊处理一下
					effective_width = effective_width - const_tylsmj.CLEARANCE * 2;
				}
				if (touch_pos.x >= -const_tylsmj.CLEARANCE && touch_pos.x < effective_width && touch_pos.y >= 0 && touch_pos.y <= hand_panel_height) {
					...
					if (touch_idx < standTileList.length) {
						touch_pos.x = touch_pos.x + const_tylsmj.CLEARANCE;
		...
		function player_hand_panel_event(sender, eventType){
            // NOTE:下乡禁止触摸和滑动
            if(self.touch_state === const_val.PLAYER_TOUCH_FORCE_STATE || player.curGameRoom.discardStateList[player.serverSitNum] === const_tylsmj.DISCARD_FORCE){
                if(self.curSelectTile){
                    reset_tile()
                }
                return;
			...
			if (eventType == ccui.Widget.TOUCH_BEGAN) {
				if (player.gameOperationAdapter.canTingTiles) {
					if (player.gameOperationAdapter.canTingTiles.length > 0) {
						if (self.curSelectTile && player.gameOperationAdapter.canTingTiles.indexOf(self.curSelectTile.tileNum) < 0 && player.curGameRoom.discardStateList[player.serverSitNum] === const_tylsmj.DISCARD_FORCE) {
							reset_tile()
							return;
						}
					}
		...
		if (this.isArrange) {
			...
			img.setContentSize(cc.size(const_tylsmj.CLEARANCE, player_hand_panel.getContentSize().height));
			img.setAnchorPoint(0, 0);
			this.rootUINode.getChildByName("player_tile_panel0").addChild(img);
			img.setPosition(-const_tylsmj.CLEARANCE + player_hand_panel.getPositionX(), 0);
	...
	mark_discard_king_tiles:function () {
        ...
        for(var i = 0; i < list.length; i++){
            ...
            let start = 0 ;
            if(tileSum > const_tylsmj.MAX_DISCARD_TILES_SIZE){
                start = tileSum - const_tylsmj.MAX_DISCARD_TILES_SIZE;
            }
	...
	mark_same_tiles:function(tileNum){
		...
		for(var i = 0; i < list.length; i++){
			...
			var maxDeskSize = const_tylsmj.MAX_DISCARD_TILES_SIZE;
			if (player.curGameRoom.discardStateList[i] === const_tylsmj.DISCARD_FORCE) {
	...
	cancel_mark_same_tiles:function(){
		...
		for(var i = 0; i < list.length; i++){
            ...
            for(var j = start; j< tileSum; j++){
                if(player.curGameRoom.kingTiles.indexOf(tiles[j]) < 0)	{
	                var tile_img = cur_player_discard_panel.getChildByName("tile_img_" + (j  - start));
	                if (tile_img.color.b === const_tylsmj.mark_ting_color.b) {
		                continue;
	                }
                    this.cancel_tile_color(tile_img);
                }
	...
	update_player_hand_tiles: function (serverSitNum, tileList) {
        ...
        for (var i = 0; i < 14; i++) {
            ...
            if(handTilesList[i] !== undefined){
                if(handTilesList[i] > 0){
                    ...
                    if(isPlayer){
	                    if(discardState === const_tylsmj.DISCARD_FORCE){
	                    	this.mark_same_tile(tile_img);
	                    	this.lock_player_hand_tiles();
	                    }else{
		                    if (tempStandTileList.indexOf(handTilesList[i]) >= 0) {
			                    ...
			                    if (this.isArrange) {
				                    tile_img.setPositionX(tile_img.getPositionX() - const_tylsmj.CLEARANCE);
			                    }
                    ...
                    if(isPlayer){
                        ...
                        if (i < standTileList.length) {
	                        ...
	                        if (!(handTilesList.length % 3 === 2 && i === handTilesList.length - 1)) {
		                        if (idx === 1) {
			                        tile_img.setPositionY(tile_img.getPositionY() + const_tylsmj.CLEARANCE);
		                        } else if (idx === 2) {
			                        tile_img.setPositionX(tile_img.getPositionX() - const_tylsmj.CLEARANCE);
		                        } else if (idx === 3) {
			                        tile_img.setPositionY(tile_img.getPositionY() - const_tylsmj.CLEARANCE);
        ...
        if(serverSitNum == player.serverSitNum && player.curGameRoom.discardStateList[serverSitNum] === const_tylsmj.DISCARD_FREE){
	...
	update_player_up_tiles: function (serverSitNum) {
		...
		if (idx === 0 && isLeftShift) {
			cur_player_tile_panel.setPositionX(-const_tylsmj.CLEARANCE);
		}
	...
	play_discard_anim:function(serverSitNum, onlyTilePointer, buckle){
        ...
        if(discardTilesSum > const_tylsmj.MAX_DISCARD_TILES_SIZE) discardIndex = const_tylsmj.MAX_DISCARD_TILES_SIZE - 1;
	...
	update_player_discard_tiles:function(serverSitNum){
		...
		var deskSize = const_tylsmj.DISCARD_TILES_SIZE;
		var maxDeskSize = const_tylsmj.MAX_DISCARD_TILES_SIZE;
		...
		var discardTilesList = player.curGameRoom.discardTilesList[serverSitNum];
		var tingTilePos = undefined;
		if (player.curGameRoom.discardStateList[serverSitNum] === const_tylsmj.DISCARD_FORCE) {
        ...
        for(var i = 0; i < deskSize; i++){
            ...
            if(i < maxDeskSize && discardTilesList[i] > 0){
	            ...
	            if (tingTilePos >= 0 && tingTilePos === i) {
                	if (isPlayer) {
		                // this.mark_same_tile(tile_img);
		                tile_img.color = const_tylsmj.mark_ting_color;
	                } else {
		                if (options.mahjongdesk) {
    ...
    update_wintips_btn:function () {
        ...
        if(canWinTiles.length <= 0 || player.curGameRoom.curPlayerSitNum == player.serverSitNum){
            ...
            wintips_btn.addTouchEventListener(function (sender, eventType) {
                if (eventType === ccui.Widget.TOUCH_BEGAN) {
                    self.update_canwin_tile_panel(const_tylsmj.WINTIPS_BTN_DISPLAY);
    ...
    update_canwin_tile_panel:function(select_tile){
        ...
        var self = this;
        var canwin_tile_panel = self.rootUINode.getChildByName("canwin_tile_panel");
        if(select_tile == const_tylsmj.NOT_DISPLAY_CANWIN_PANEL){
            canwin_tile_panel.setVisible(false);
            return;
        }
        var player = h1global.player();
        if (player.curGameRoom.standTileList[player.serverSitNum].indexOf(select_tile) < 0 && select_tile !== const_tylsmj.WINTIPS_BTN_DISPLAY) {
	        canwin_tile_panel.setVisible(false);
        	return;
        }
    ...
    play_result_anim: function (player_info_list) {
        ...
        for (var i = 0; i < curGameRoom.player_num; i++) {
			...
			cutil_tylsmj.tileSort(handTiles, player.curGameRoom.kingTiles);
    ...
    update_operation_panel:function(op_dict, from_type){
        ...
        var self = this;
        var player = h1global.player();
        if (player.curGameRoom.discardStateList[player.serverSitNum] === const_tylsmj.DISCARD_FORCE && player.curGameRoom.game_mode !== const_tylsmj.KING_GAME_MODE) {
