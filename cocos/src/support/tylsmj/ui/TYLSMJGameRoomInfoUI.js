var TYLSMJGameRoomInfoUI = MultipleLayoutUI.extend({

	ctor: function () {
		this._super();
		this.setLocalZOrder(const_val.GameConfigZOrder - 1);
	},
	...
	show_by_info: function (resourceFilename) {
		if (resourceFilename) {
			...
			this.resourceFilename = cc.sys.localStorage.getItem(cutil.keyConvert("GAME_ROOM_UI", h1global.curUIMgr.gameType)) == const_val.GAME_ROOM_2D_UI ? TYLSMJGameRoomInfoUI.ResourceFile2D : TYLSMJGameRoomInfoUI.ResourceFile3D;
		}
		this.show()
	},
	...
	initUI: function() {
		...
		if (this.resourceFilename === TYLSMJGameRoomInfoUI.ResourceFile2D) {
			this.updateLayout2D();
		} else if (this.resourceFilename === TYLSMJGameRoomInfoUI.ResourceFile3D) {
			this.updateLayout3D();
		} else {
...
TYLSMJGameRoomInfoUI.ResourceFile2D = "res/ui/TYLSMJGameRoomInfo2DUI.json";
TYLSMJGameRoomInfoUI.ResourceFile3D = "res/ui/TYLSMJGameRoomInfoUI.json";
TYLSMJGameRoomInfoUI.getResourceFile = function(gameType) {
	if (gameType == const_val.GAME_ROOM_2D_UI) {
		return TYLSMJGameRoomInfoUI.ResourceFile2D;
	} else if (gameType == const_val.GAME_ROOM_3D_UI) {
		return TYLSMJGameRoomInfoUI.ResourceFile3D;
	}else{
