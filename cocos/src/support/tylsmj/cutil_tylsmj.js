"use strict";
var cutil_tylsmj = function(){}

cutil_tylsmj.get_count = function(tiles, t){
	var sum = 0;
...
cutil_tylsmj.meld_history = {};

cutil_tylsmj.meld_with_pair_need_num = function(tiles, history, used) {
	history = history || this.meld_history;
	var case1 = 999;
	var case2 = 999;
	var idx = -1;

	if (cutil_tylsmj.meld_only_need_num(tiles, history) === 0){
		case1 = 2;
	}
	...
	for(var i = 0; i < tiles.length; i++){
		...
		if (cutil_tylsmj.get_count(tiles, tiles[i]) === 1){
			idx = tmp.indexOf(tiles[i]);
			tmp.splice(idx, 1);
			case2 = Math.min(case2, 1 + cutil_tylsmj.meld_only_need_num(tmp, history));
		} else {
			idx = tmp.indexOf(tiles[i]);
			tmp.splice(idx, 1);
			idx = tmp.indexOf(tiles[i]);
			tmp.splice(idx, 1);
			case2 = Math.min(case2, cutil_tylsmj.meld_only_need_num(tmp, history));
		}
	}
...
cutil_tylsmj.meld_only_need_num = function(tiles, history, used){
	history = history || this.meld_history;
	used = used || 0;
	if (used > 4){
		return 999;
...
cutil_tylsmj.canTenPai = function (handTiles, kingTiles) {
    kingTiles = kingTiles || [];
    ...
    // 先处理特殊情况
    // 7对胡
	if (cutil_tylsmj.checkIs7Pairs(handTiles, handTilesButKing, kingTilesNum + 1)) {
    	return true;
	}
...
cutil_tylsmj.checkIs7Pairs = function(handTilesButKing, kingTilesNum){
    if (handTilesButKing.length + kingTilesNum !== 14) {return false}
    var tileDict = cutil_tylsmj.getTileNumDict(handTilesButKing);
    var need_num = 0
...
// 十三幺
cutil_tylsmj.getThirteenOrphans = function (handTilesButKing, kingTilesNum) {
	...
	function withoutKings() {
		...
		for (var i = 0; i < need_tiles.length; i++) {
			if (cutil_tylsmj.get_count(handTilesButKing, need_tiles[i]) === 2) {
				return true;
			}
		}
		return false;
...
cutil_tylsmj.checkIsDragon = function(handTiles){
...
cutil_tylsmj.checkIs6Pairs = function(tilesButKing, kingTilesNum){
    if (tilesButKing.length + kingTilesNum !== 12) {return false}
    var tileDict = cutil_tylsmj.getTileNumDict(tilesButKing);
    var need_num = 0
    for(var tile in tileDict){
        need_num += (tileDict[tile] % 2);
    }
...
cutil_tylsmj.canNormalWinWithKing3N2 = function (handTilesButKing, kingTilesNum) {
...
cutil_tylsmj.canNormalWinWithKing3N = function (handTilesButKing, kingTilesNum) {
...
cutil_tylsmj.canNormalWinWithoutKing3N2 = function (handTiles) {
...
cutil_tylsmj.isMeld = function (tiles) {
	if (tiles.length % 3 !== 0) {
		return false;
	}
...
cutil_tylsmj.isMeldWithPair = function (tiles) {
    if (tiles.length % 3 !== 2) {
        return false;
    }
    var total = 0;
...
cutil_tylsmj.checkMeldInPossible = function (tiles, possibleList) {
	var idx;
...
cutil_tylsmj.sortChowTileList = function (chowTile, tiles) {
...
cutil_tylsmj.tileSort = function (tiles, kingTiles, standTileList) {
...
cutil_tylsmj.classifyTiles = function(tiles, kingTiles){
	kingTiles = kingTiles || [];
    var kings = [];
    var chars = [];
    var bambs = [];
...
cutil_tylsmj.classifyKingTiles = function(tiles, kingTiles){
    kingTiles = kingTiles || [];
    var kings = [];
    var others = [];
    for (var i = 0; i < tiles.length; i++) {
...
//获取同样牌的张数 dict
cutil_tylsmj.getTileNumDict = function(tiles){
	var tileDict = {}
	for (var i = 0; i < tiles.length; i++) {
   		var t = tiles[i]
   		if (!tileDict[t]) {
...
cutil_tylsmj.count = function(list, key){
	var dict = {}
...
cutil_tylsmj.tileSortFunc = function(a, b){
	if(a == b){
		return 0;
	}
