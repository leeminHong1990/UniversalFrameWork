...
"use strict"
var FYQYMMJGameRoomUI = UIBase.extend({
	ctor: function () {
		this._super();
		this.talk_img_num = 0;
	},
	initUI: function () {
		...
		if (player && player.curGameRoom.player_num == 3) {
			cutil.swap_node_name(this.rootUINode.getChildByName("player_info_panel2"), this.rootUINode.getChildByName("player_info_panel3"));
			cutil.swap_node_name(this.rootUINode.getChildByName("player_tile_panel2"), this.rootUINode.getChildByName("player_tile_panel3"));
			cutil.swap_node_name(this.rootUINode.getChildByName("game_info_panel").getChildByName("player_discard_panel2"), this.rootUINode.getChildByName("game_info_panel").getChildByName("player_discard_panel3"));

			this.rootUINode.getChildByName("player_info_panel3").setVisible(false);
			this.rootUINode.getChildByName("player_tile_panel3").setVisible(false);
			this.rootUINode.getChildByName("game_info_panel").getChildByName("player_discard_panel3").setVisible(false);
		}
		if (player && player.curGameRoom.player_num == 2) {
			cutil.swap_node_name(this.rootUINode.getChildByName("player_info_panel2"), this.rootUINode.getChildByName("player_info_panel1"));
			cutil.swap_node_name(this.rootUINode.getChildByName("player_tile_panel2"), this.rootUINode.getChildByName("player_tile_panel1"));
			cutil.swap_node_name(this.rootUINode.getChildByName("game_info_panel").getChildByName("player_discard_panel2"), this.rootUINode.getChildByName("game_info_panel").getChildByName("player_discard_panel1"));
	...
	init_ting_panel: function () {
		...
		function ting_btn_event(sender, eventType) {
			if (eventType === ccui.Widget.TOUCH_ENDED) {
				...
				// player.setDiscardState(const_fyqymmj.DISCARD_FORCE);
				// self.checkTing = true;
				h1global.curUIMgr.roomLayoutMgr.iterUI(function (ui) {
					ui.checkTing = true;
				});
		...
		function guo_btn_event(sender, eventType) {
			if (eventType === ccui.Widget.TOUCH_ENDED) {
				var player = h1global.entityManager.player();
				player.setDiscardState(const_fyqymmj.DISCARD_FREE);
				h1global.curUIMgr.roomLayoutMgr.iterUI(function (ui) {
					ui.checkTing = false;
				});
	...
	_setTileMask: function () {
		...
		for (var i = 0; i < handTiles.length; i++) {
			...
			if (player.gameOperationAdapter.canTingTiles.indexOf(handTiles[i]) < 0) {
				tile_img.color = const_fyqymmj.mark_same_color;
			}
		}
	},
	...
	_setTileNoMask: function () {
		...
		for (var i = 0; i < handTiles.length; i++) {
			var tile_img = cur_player_hand_panel.getChildByName("tile_img_" + i.toString());
			tile_img.color = const_fyqymmj.mark_none_color;
		}
	},
	...
	_resetSelectTile: function () {
		...
		if (this.curSelectTile) {
			...
			this.update_canwin_tile_panel(const_fyqymmj.NOT_DISPLAY_CANWIN_PANEL);
		}
	},
	...
	init_player_tile_panel: function () {
		...
		idx = cutil.change_idx(player.curGameRoom.player_num, idx);
		...
		function unchoose_tile() {
			...
			self.update_canwin_tile_panel(const_fyqymmj.NOT_DISPLAY_CANWIN_PANEL);
		}
		...
		function discard_tile(tile, discard_pos) {
			...
			if (player) {
				...
				if (self.checkTing && player.gameOperationAdapter.canTingTiles.indexOf(tile.tileNum) >= 0) {
					player.setDiscardState(const_fyqymmj.DISCARD_FORCE, tile.tileNum);
		...
		function player_hand_panel_event(sender, eventType) {
			// NOTE:下乡禁止触摸和滑动
			if (self.touch_state === const_val.PLAYER_TOUCH_FORCE_STATE || player.curGameRoom.discardStateList[player.serverSitNum] === const_fyqymmj.DISCARD_FORCE) {
				if (self.curSelectTile) {
					reset_tile()
				}
				return;
			...
			if (eventType == ccui.Widget.TOUCH_BEGAN) {
				if (player.gameOperationAdapter.canTingTiles) {
					if (player.gameOperationAdapter.canTingTiles.length > 0) {
						if (self.curSelectTile && player.gameOperationAdapter.canTingTiles.indexOf(self.curSelectTile.tileNum) < 0 && player.curGameRoom.discardStateList[player.serverSitNum] === const_fyqymmj.DISCARD_FORCE) {
							reset_tile()
							return;
						}
					}
	...
	mark_king_tile: function (tile_img) {
		tile_img.color = const_fyqymmj.mark_king_color;
	},
	...
	mark_discard_king_tiles: function () {
		...
		for (var i = 0; i < list.length; i++) {
			...
			let start = 0;
			if (tileSum > const_fyqymmj.MAX_DISCARD_TILES_SIZE) {
				start = tileSum - const_fyqymmj.MAX_DISCARD_TILES_SIZE;
			}
	...
	mark_same_tiles: function (tileNum) {
		...
		for (var i = 0; i < list.length; i++) {
			...
			var maxDeskSize = const_fyqymmj.MAX_DISCARD_TILES_SIZE;
			if (player.curGameRoom.discardStateList[i] === const_fyqymmj.DISCARD_FORCE) {
			...
			for (var j = start; j < tileSum; j++) {
				if (tiles[j] == tileNum) {
					if (tingTilePos >= 0 && tingTilePos === j) {
						continue;
					}
					var tile_img = cur_player_discard_panel.getChildByName("tile_img_" + (j - start));
					this.mark_same_tile(tile_img);
	...
	cancel_mark_same_tiles: function () {
		...
		for (var i = 0; i < list.length; i++) {
			...
			for (var j = start; j < tileSum; j++) {
				if (player.curGameRoom.kingTiles.indexOf(tiles[j]) < 0) {
					var tile_img = cur_player_discard_panel.getChildByName("tile_img_" + (j - start));
					if (tile_img.color.b === const_fyqymmj.mark_ting_color.b) {
						continue;
					}
					this.cancel_tile_color(tile_img);
				}
		...
		function playAnimation() {
			...
			var step2 = cc.Sequence.create(cc.DelayTime.create(0.1), cc.CallFunc.create(function () {
					...
					for (var i = 0; i < player.curGameRoom.player_num; i++) {
						...
						i = cutil.change_idx(player.curGameRoom.player_num, i);
	...
	update_player_hand_tiles: function (serverSitNum, tileList) {
		...
		for (var i = 0; i < 14; i++) {
			...
			if (handTilesList[i] !== undefined) {
				if (handTilesList[i] > 0) {
					...
					if (isPlayer) {
						if (discardState === const_fyqymmj.DISCARD_FORCE) {
							this.mark_same_tile(tile_img);
							this.lock_player_hand_tiles();
						} else {
							if(!this.checkTing){
		...
		if (serverSitNum == player.serverSitNum && player.curGameRoom.discardStateList[serverSitNum] === const_fyqymmj.DISCARD_FREE) {
	...
	update_player_up_tiles: function (serverSitNum) {
		...
		for (var i = 0; i < curUpNum; i++) {
			...
			if (ops) {
				...
				from_idx = cutil.change_idx(player.curGameRoom.player_num, from_idx);
	...
	play_discard_anim: function (serverSitNum, onlyTilePointer, buckle) {
		...
		if (discardTilesSum > const_fyqymmj.MAX_DISCARD_TILES_SIZE) discardIndex = const_fyqymmj.MAX_DISCARD_TILES_SIZE - 1;
	...
	update_player_discard_tiles: function (serverSitNum) {
		...
		var deskSize = const_fyqymmj.DISCARD_TILES_SIZE;
		var maxDeskSize = const_fyqymmj.MAX_DISCARD_TILES_SIZE;
		...
		var discardTilesList = player.curGameRoom.discardTilesList[serverSitNum];
		var tingTilePos = undefined;
		if (player.curGameRoom.discardStateList[serverSitNum] === const_fyqymmj.DISCARD_FORCE) {
		...
		for (var i = 0; i < deskSize; i++) {
			...
			if (i < maxDeskSize && discardTilesList[i] > 0) {
				...
				mahjong_img.ignoreContentAdaptWithSize(true);
				if (options.mahjong) {
					if (options.mahjong.positionX) {
						mahjong_img.setPositionX(options.mahjong.positionX[(i + 10) % 10]);
					}
					if (options.mahjong.flippedX !== undefined) {
						mahjong_img.setFlippedX(options.mahjong.flippedX);
					}
					if (options.mahjong.flippedY !== undefined) {
						mahjong_img.setFlippedY(options.mahjong.flippedY);
					}
					if (options.mahjong.flippedX_list) {
						mahjong_img.setFlippedX(options.mahjong.flippedX_list[(i + 10) % 10]);
					}
					if (options.mahjong.scale) {
						mahjong_img.setScale(options.mahjong.scale[0], options.mahjong.scale[1]);
					}
					if (options.mahjong.rotationX) {
						mahjong_img.setRotationX(options.mahjong.rotationX[(i + 10) % 10]);
					}
					if (options.mahjong.skewY) {
						mahjong_img.setSkewY(options.mahjong.skewY);
					}
				} else {
				...
				if (tingTilePos >= 0 && tingTilePos === i) {
					if (isPlayer) {
						// this.mark_same_tile(tile_img);
						tile_img.color = const_fyqymmj.mark_ting_color;
					} else {
						if (options.mahjongdesk) {
	...
	update_wintips_btn: function () {
		...
		if (canWinTiles.length <= 0 || player.curGameRoom.curPlayerSitNum == player.serverSitNum) {
			...
			wintips_btn.addTouchEventListener(function (sender, eventType) {
				if (eventType === ccui.Widget.TOUCH_BEGAN) {
					self.update_canwin_tile_panel(const_fyqymmj.WINTIPS_BTN_DISPLAY);
	...
	update_canwin_tile_panel: function (select_tile) {
		...
		var self = this;
		var canwin_tile_panel = self.rootUINode.getChildByName("canwin_tile_panel");
		if (select_tile == const_fyqymmj.NOT_DISPLAY_CANWIN_PANEL) {
			canwin_tile_panel.setVisible(false);
			return;
		}
		var player = h1global.player();
	...
	playOperationFunc: function (curSitNum, opId) {
		...
		function opPos(curSitNum, cur_img) {
			if (curSitNum == 0) {
				...
				if (h1global.player().curGameRoom.player_num === 2) {
					cur_img.setPosition(cc.p(cc.winSize.width * 0.5, self.rootUINode.getChildByName("player_tile_panel1").getPositionY()-160));
				}
			} else if (curSitNum == 2) {
				cur_img.setPosition(cc.p(cc.winSize.width * 0.5, self.rootUINode.getChildByName("player_tile_panel2").getPositionY() - 160));
				if (h1global.player().curGameRoom.player_num === 3) {
					cur_img.setPosition(cc.p(self.rootUINode.getChildByName("player_tile_panel2").getPositionX(), cc.winSize.height * 0.5));
				}
			} else if (curSitNum == 3) {
	...
	getMsgPos: function (playerInfoPanel, idx) {
		return emotion.getMsgPos(playerInfoPanel, idx);
	},

	playMessageAnim: function (serverSitNum, msg) {
		emotion.playMessageAnim(this,serverSitNum, msg);
	},

	getExpressionPos: function (player_info_panel, idx) {
		return emotion.getExpressionPos(player_info_panel, idx);
	},
	...
	playVoiceAnim: function (serverSitNum, record_time) {
		emotion.playVoiceAnim(this,serverSitNum, record_time);
	},
	...
	play_luckytiles_anim: function (luckyTileList, callback) {
		...
		for (var i = 0; i < 2; i++) {
			...
			if (luckyTileNum) {
				...
				mahjong_img.setVisible(false);
				tile_img.runAction(cc.Sequence.create(cc.DelayTime.create(0.4 * (i + 1)), cc.CallFunc.create((function (luckyTileNum, tile_img, mahjong_img) {
					return function () {
						tile_img.loadTexture("Mahjong/mahjong_tile_player_hand.png", ccui.Widget.PLIST_TEXTURE);
						mahjong_img.ignoreContentAdaptWithSize(true);
	...
	play_result_anim: function (player_info_list) {
		...
		for (var i = 0; i < curGameRoom.player_num; i++) {
			...
			cutil_fyqymmj.tileSort(handTiles, player.curGameRoom.kingTiles);
	...
	update_operation_panel: function (op_dict, from_type) {
		...
		var self = this;
		var player = h1global.player();
		// if (player.curGameRoom.discardStateList[player.serverSitNum] === const_fyqymmj.DISCARD_FORCE && player.curGameRoom.game_mode !== const_fyqymmj.KING_GAME_MODE) {
