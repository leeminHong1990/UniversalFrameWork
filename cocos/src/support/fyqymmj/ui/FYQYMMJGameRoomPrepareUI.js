...
"use strict";
var FYQYMMJGameRoomPrepareUI = UIBase.extend({
	ctor: function () {
		this._super();
		this.resourceFilename = "res/ui/FYQYMMJGameRoomPrepareUI.json";
		this.talk_img_num = 0;
	},
	...
	initUI: function () {
		...
		if (player && player.curGameRoom.player_num == 2 && player.curGameRoom.curRound > 0) {
			cutil.swap_node_name(this.gameprepare_panel.getChildByName("player_info_panel2"), this.gameprepare_panel.getChildByName("player_info_panel1"));
		}
		if (player && player.curGameRoom.player_num == 3 && player.curGameRoom.curRound > 0) {
			cutil.swap_node_name(this.gameprepare_panel.getChildByName("player_info_panel2"), this.gameprepare_panel.getChildByName("player_info_panel3"));
		}
		...
		//var share_title = ' ' + switches.gameName + (player.curGameRoom.club_id > 0 ? '亲友圈号【' + player.curGameRoom.club_id.toString() + '】' : '房间号【' + player.curGameRoom.roomID.toString() + '】');
		// var desk_num = player.curGameRoom.table_idx+1;
		// var share_title = ' 推倒胡 '+ (player.curGameRoom.club_id > 0 ? '亲友圈号【' + player.curGameRoom.club_id.toString() + '】'+ desk_num +'号桌' : '房间号【' + player.curGameRoom.roomID.toString() + '】');
		// var share_title = ' 房间号【' + player.curGameRoom.roomID.toString() + '】,招募群主,1000红包奖励群主!';

		// var share_title = cutil.get_share_title(player.curGameRoom,const_val.FenYangQYMMJ);
		var share_title = '';

		//var share_desc = this.get_share_desc();
		var share_desc = cutil.get_share_desc(player.curGameRoom, const_val.FenYangQYMMJ);
		...
		wxinvite_btn.addTouchEventListener(function (sender, eventType) {
			if (eventType == ccui.Widget.TOUCH_ENDED) {
				...
				share_title = cutil.get_share_title(player.curGameRoom, const_val.FenYangQYMMJ);
		...
		gameroomUIMgr.adapt_gameprepare_panel(this.gameprepare_panel, player.curGameRoom.player_num, 4);
	},
	...
	get_share_desc: function () {
		...
		if (player.curGameRoom.add_winds === 1) {
			share_list.push("带风");
		}
		if (player.curGameRoom.add_dealer === 1) {
			share_list.push("带庄");
		}
		if (player.curGameRoom.kong_follow_win === 1) {
			share_list.push("杠随胡");
		}
		if (player.curGameRoom.need_ting === 1) {
			share_list.push("报听");
		}
		if (player.curGameRoom.bao_hu === 1) {
			share_list.push("包胡");
		}
		if (player.curGameRoom.multiplayer_win === 1) {
			share_list.push("一炮多响");
		}
		if (player.curGameRoom.king_mode === 1) {
			share_list.push("耗子");
		}
		if (player.curGameRoom.lack_door === 1) {
			share_list.push("缺一门");
		}
	...
	update_location: function () {
		...
		for (var i = 0; i < ip_list.length; i++) {
			for (var j = 0; j < ip_list.length; j++) {
				...
				if (ip_list[i] == ip_list[j]) {
					if (idx_ip_list.indexOf(i) < 0) {
						idx_ip_list.push(i);
					}
					if (idx_ip_list.indexOf(j) < 0) {
						idx_ip_list.push(j);
					}
				}
			}
		...
		for (var i = 0; i < distance_list.length; i++) {
			for (var j = 0; j < distance_list.length; j++) {
				if (i == j || i == player.serverSitNum || j == player.serverSitNum) {
					continue;
				}
				if (distance_list[i][j] < 100 && distance_list[i][j] >= 0) {
					if (idx_distance_list.indexOf(i) < 0) {
						idx_distance_list.push(i);
					}
					if (idx_distance_list.indexOf(j) < 0) {
						idx_distance_list.push(j);
					}
				}
			}
		}
