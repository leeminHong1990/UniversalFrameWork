"use strict";
var cutil_fyqymmj = function(){}

cutil_fyqymmj.get_count = function(tiles, t){
	var sum = 0;
...
cutil_fyqymmj.meld_history = {};

cutil_fyqymmj.meld_with_pair_need_num = function(tiles, history, used) {
	history = history || this.meld_history;
	var case1 = 999;
	var case2 = 999;
	var idx = -1;

	if (cutil_fyqymmj.meld_only_need_num(tiles, history) === 0){
		case1 = 2;
	}
	...
	for(var i = 0; i < tiles.length; i++){
		...
		if (cutil_fyqymmj.get_count(tiles, tiles[i]) === 1){
			idx = tmp.indexOf(tiles[i]);
			tmp.splice(idx, 1);
			case2 = Math.min(case2, 1 + cutil_fyqymmj.meld_only_need_num(tmp, history));
		} else {
			idx = tmp.indexOf(tiles[i]);
			tmp.splice(idx, 1);
			idx = tmp.indexOf(tiles[i]);
			tmp.splice(idx, 1);
			case2 = Math.min(case2, cutil_fyqymmj.meld_only_need_num(tmp, history));
		}
	}
...
cutil_fyqymmj.meld_only_need_num = function(tiles, history, used){
	history = history || this.meld_history;
	used = used || 0;
	if (used > 4){
		return 999;
...
cutil_fyqymmj.canTenPai = function (handTiles, kingTiles) {
    kingTiles = kingTiles || [];
    ...
    // 先处理特殊情况
    // 7对胡
	if (cutil_fyqymmj.checkIs7Pairs(handTiles, handTilesButKing, kingTilesNum + 1)) {
    	return true;
	}
...
cutil_fyqymmj.checkIs7Pairs = function(handTilesButKing, kingTilesNum){
    if (handTilesButKing.length + kingTilesNum !== 14) {return false}
    var tileDict = cutil_fyqymmj.getTileNumDict(handTilesButKing);
    var need_num = 0
...
// 十三幺
cutil_fyqymmj.getThirteenOrphans = function (handTilesButKing, kingTilesNum) {
	...
	function withoutKings() {
		...
		for (var i = 0; i < need_tiles.length; i++) {
			if (cutil_fyqymmj.get_count(handTilesButKing, need_tiles[i]) === 2) {
				return true;
			}
		}
		return false;
...
//清一色的判断，无论有无癞子都可以，前提是可胡牌型
//handTiles是手牌，uptiles是碰杠的牌
cutil_fyqymmj.getTileColorType = function (handTiles, uptiles, kingTiles) {
	kingTiles = kingTiles || [];
	var suitNumList = [0, 0, 0];
	var honorNum = 0;
	for (var i = 0 ; i < handTiles.length ; i++) {
		if (kingTiles.indexOf(handTiles[i]) >= 0) {
			continue;
		} else if (const_val.CHARACTER.indexOf(handTiles[i]) >= 0) {
			suitNumList[0] = 1;
		} else if (const_val.BAMBOO.indexOf(handTiles[i]) >= 0) {
			suitNumList[1] = 1;
		} else if (const_val.DOT.indexOf(handTiles[i]) >= 0) {
			suitNumList[2] = 1;
		} else if (const_val.DRAGONS.indexOf(handTiles[i]) >= 0 || const_val.WINDS.indexOf(handTiles[i]) >= 0) {
			honorNum = 1;
		}
	}
	for (var j = 0 ; j < uptiles.length ; j++) {
		...
		for (var k = 0 ; k < meld.length ; k++) {
			if (kingTiles.indexOf(meld[i]) >= 0) {
				continue;
			} else if (const_val.CHARACTER.indexOf(meld[k]) >= 0) {
				suitNumList[0] = 1;
			} else if (const_val.BAMBOO.indexOf(meld[k]) >= 0) {
				suitNumList[1] = 1;
			} else if (const_val.DOT.indexOf(meld[k]) >= 0) {
				suitNumList[2] = 1;
			} else if (const_val.DRAGONS.indexOf(meld[k]) >= 0 || const_val.WINDS.indexOf(meld[k]) >= 0) {
				honorNum = 1;
			}
		}
	}
...
// 一条龙
// 传进来的手牌 必须经过 this.classifyTiles 排序
cutil_fyqymmj.checkIsDragon = function(handTiles){
   // kingTilesNum = kingTilesNum || 0;
	//cc.log(handTilesButKing.length+ kingTilesNum);
    if (handTiles.length !== 14) {return false}

    //cc.log("进来了");
    //var tileDict = cutil_fyqymmj.getTileNumDict(handTilesButKing);
    var need_num = 0;
    for(var tile in handTiles){
        if (const_val.CHARACTER.indexOf(handTiles[tile])<0&&const_val.BAMBOO.indexOf(handTiles[tile])<0&&const_val.DOT.indexOf(handTiles[tile])<0){
            if(need_num!=8) {
                need_num = 0;
                continue;
            }
		}
        if (handTiles[Number(tile)+1]==handTiles[tile]+1){
            need_num++;
            continue;
		}
...
cutil_fyqymmj.checkIs6Pairs = function(tilesButKing, kingTilesNum){
    if (tilesButKing.length + kingTilesNum !== 12) {return false}
    var tileDict = cutil_fyqymmj.getTileNumDict(tilesButKing);
    var need_num = 0
    for(var tile in tileDict){
        need_num += (tileDict[tile] % 2);
    }
...
cutil_fyqymmj.canNormalWinWithKing3N2 = function (handTilesButKing, kingTilesNum) {
...
cutil_fyqymmj.canNormalWinWithKing3N = function (handTilesButKing, kingTilesNum) {
...
cutil_fyqymmj.canNormalWinWithoutKing3N2 = function (handTiles) {
...
cutil_fyqymmj.isMeld = function (tiles) {
	if (tiles.length % 3 !== 0) {
		return false;
	}
...
cutil_fyqymmj.isMeldWithPair = function (tiles) {
    if (tiles.length % 3 !== 2) {
        return false;
    }
    var total = 0;
...
cutil_fyqymmj.checkMeldInPossible = function (tiles, possibleList) {
	var idx;
...
cutil_fyqymmj.sortChowTileList = function (chowTile, tiles) {
...
cutil_fyqymmj.tileSort = function (tiles, kingTiles, standTileList) {
	...
	tiles.sort(function (a,b) {
		if(kings.indexOf(a) >= 0 && kings.indexOf(b) >= 0){
			...
			return 1
		} else if(standTileList.indexOf(a) >= 0 && standTileList.indexOf(b) >= 0){
			return a-b;
		} else if(standTileList.indexOf(a) >= 0){
			return -1;
		} else if(standTileList.indexOf(b) >= 0){
			return 1;
...
cutil_fyqymmj.classifyTiles = function(tiles, kingTiles){
	kingTiles = kingTiles || [];
    var kings = [];
    var chars = [];
    var bambs = [];
...
cutil_fyqymmj.classifyKingTiles = function(tiles, kingTiles){
    kingTiles = kingTiles || [];
    var kings = [];
    var others = [];
    for (var i = 0; i < tiles.length; i++) {
...
//获取同样牌的张数 dict
cutil_fyqymmj.getTileNumDict = function(tiles){
	var tileDict = {}
	for (var i = 0; i < tiles.length; i++) {
   		var t = tiles[i]
   		if (!tileDict[t]) {
...
cutil_fyqymmj.count = function(list, key){
	var dict = {}
...
cutil_fyqymmj.tileSortFunc = function(a, b){
	if(a == b){
		return 0;
	}
