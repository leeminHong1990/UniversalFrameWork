...
/*-----------------------------------------------------------------------------------------
 interface
 -----------------------------------------------------------------------------------------*/
var FYQYMMJGameRules = FYQYMMJRoomOperationAdapter.extend({
    ...
    getDrawOpDict: function (drawTile, serverSitNum) {
        ...
        if (this.isOpLimit()) {
	        ...
	        if (this.curGameRoom.need_ting === 1) {
		        if (handTiles.length % 3 === 2 && this.canWin(handTiles, drawTile, serverSitNum, const_val.OP_DRAW_WIN) && this.curGameRoom.discardStateList[serverSitNum] === const_fyqymmj.DISCARD_FORCE) {
        ...
        //暗杠
        var tile2NumDict = cutil_fyqymmj.getTileNumDict(handTiles)
    ...
    canWin:function(handTiles, finalTile, serverSitNum, aid){
        ...
        if(this.nomalCanWin(handTiles, finalTile, serverSitNum, aid)){
            ...
            // 过胡只能自摸
            if (aid !== const_val.OP_DRAW_WIN && this.curGameRoom.pass_win_list[this.serverSitNum] === const_fyqymmj.PASS_WIN) {
                return false;
            }
    ...
    nomalCanWin: function (handTiles, finalTile, serverSitNum, aid) {
        ...
        var kingClassified = cutil_fyqymmj.classifyKingTiles(handCopyTile, this.curGameRoom.kingTiles);
        var kings = kingClassified[0];
        var handTilesButKing  = kingClassified[1];
        var kingTilesNum = kings.length;

        if (this.curGameRoom.game_mode === const_fyqymmj.SPECIAL_GAME_MODE && this.curGameRoom.shisan_yao === 1 && cutil_fyqymmj.getThirteenOrphans(handTilesButKing, kingTilesNum)) { // 十三幺
			return true;
		}

		var colorType = cutil_fyqymmj.getTileColorType(handTiles, uptiles, this.curGameRoom.kingTiles);
	    ...
	    // 	return true;
	    // }
        if (cutil_fyqymmj.checkIs7Pairs(handTilesButKing, kingTilesNum)) {              // 7对
            return true;
        }else if(kingTilesNum > 0){                                            // 有癞子
	        if (cutil_fyqymmj.canNormalWinWithKing3N2(handTilesButKing, kingTilesNum)) {
		        return true;
	        }
        } else if (cutil_fyqymmj.canNormalWinWithoutKing3N2(handTilesButKing, kingTilesNum)) { // 没癞子
	        return true;
        }
    },
