...
/*-----------------------------------------------------------------------------------------
												interface
-----------------------------------------------------------------------------------------*/
var FYQYMMJGameOperationAdapter = FYQYMMJPlaybackOperationAdapter.extend({

	// startGame : function(dealerIdx, tileList, wreathsList, kingTiles, prevailing_wind, playerWindList, diceList, swap_list){
	startGame : function(startInfo){
        ...
        if (this.serverSitNum == dealerIdx) {
			...
			cutil_fyqymmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], kingTiles);
    ...
    removeHandTile: function (serverSitNum, tileNum, count , sort) {
        ...
        if(sort){
            cutil_fyqymmj.tileSort(this.curGameRoom.handTilesList[serverSitNum], this.curGameRoom.kingTiles)
        }
    },
	...
	postOperation : function(serverSitNum, aid, tileList, buckle){
		...
		buckle = buckle || 0;
        var voice_root = "res/sound/" + cc.sys.localStorage.getItem("VOICE_LANGUAGE") + "_voice/";
		...
		if(aid == const_val.OP_PASS){
			...
			if(this.serverSitNum == serverSitNum){
				...
				if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
					...
					//轮到自己摸牌，是否可以随意出牌
					if (this.curGameRoom.discardStateList[this.serverSitNum] === const_fyqymmj.DISCARD_FORCE) {
			...
			if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
				...
				h1global.curUIMgr.roomLayoutMgr.notifyObserver("update_canwin_tile_panel", const_fyqymmj.NOT_DISPLAY_CANWIN_PANEL);
			}
			// if(this.curGameRoom.kingTiles.indexOf(tileList[0]) >= 0){
			if(buckle === 1) {
				if(this.curGameRoom.playerInfoList[serverSitNum]["sex"] == 1){
			...
			if(this.serverSitNum == serverSitNum){
                ...
                cutil_fyqymmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], this.curGameRoom.kingTiles);
                var sortChowTiles = cutil_fyqymmj.sortChowTileList(tileList[0], tileList.concat());
                // cutil_fyqymmj.tileSort(sortChowTiles, this.curGameRoom.kingTiles);
	...
	doOperation : function(aid, tileList){
		...
		if(this.curGameRoom.curPlayerSitNum === this.serverSitNum && aid === const_val.OP_PASS){
			...
			if (this.checkPassWin) {
				this.setPassWinState(const_fyqymmj.PASS_WIN);
				this.checkPassWin = false;
			}
			// return;
		}
		...
		if (this.curGameRoom.discardStateList[this.serverSitNum] === const_fyqymmj.DISCARD_FREE) {
	...
	waitForOperation : function(aid_list, tileList){
		...
		// 摸打模式
		if ((aid_list.indexOf(const_val.OP_DRAW_WIN) >= 0 || aid_list.indexOf(const_val.OP_KONG_WIN) >= 0 || aid_list.indexOf(const_val.OP_GIVE_WIN) >= 0)) {
