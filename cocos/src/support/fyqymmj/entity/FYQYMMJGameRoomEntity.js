...
var FYQYMMJGameRoomEntity = GameRoomEntity.extend({
	ctor: function (player_num, gameType) {
		...
		this.discardStateList = [const_fyqymmj.DISCARD_FREE, const_fyqymmj.DISCARD_FREE, const_fyqymmj.DISCARD_FREE, const_fyqymmj.DISCARD_FREE]
		...
		this.pass_win_list = [const_fyqymmj.NO_PASS_WIN, const_fyqymmj.NO_PASS_WIN, const_fyqymmj.NO_PASS_WIN, const_fyqymmj.NO_PASS_WIN]; //玩家是否过胡
		...
		for (var i = 0; i < recRoomInfo["player_advance_info_list"].length; i++) {
			...
			for (var j = 0; j < curPlayerInfo["op_list"].length; j++) {
				...
				if (op_info["opId"] === const_val.OP_PONG) {
					...
					sortTiles = cutil_fyqymmj.sortChowTileList(sortTiles[0], sortTiles);
					// cutil_fyqymmj.tileSort(sortTiles, this.kingTiles);
	...
	updateRoomData: function (roomInfo) {
		...
		this.guo_hu = roomInfo["guo_hu"];
		this.san_hua = roomInfo["san_hua"];
		this.shisan_yao = roomInfo["shisan_yao"];
	...
	getRoomCreateDict: function () {
		return {
			...
			"guo_hu": this.guo_hu,
			"san_hua": this.san_hua,
			"shisan_yao": this.shisan_yao,
	...
	startGame: function (kingTiles, wreathsList) {
		...
		this.pass_win_list = [const_fyqymmj.NO_PASS_WIN, const_fyqymmj.NO_PASS_WIN, const_fyqymmj.NO_PASS_WIN, const_fyqymmj.NO_PASS_WIN]; //玩家是否过胡
		this.tingTileList = [-1, -1, -1, -1];
		this.leftTileNum = gameroomUIMgr.get_sum_tile_num(this.player_num, this.game_mode == 2 ? 1 : 0);
