"use strict";
var cutil_lsbmzmj = function(){};

cutil_lsbmzmj.get_count = function(tiles, t){
    var sum = 0;
...
cutil_lsbmzmj.meld_history = {};

cutil_lsbmzmj.meld_with_pair_need_num = function(tiles, history, used) {
    history = history || this.meld_history;
    var case1 = 999;
    var case2 = 999;
    var idx = -1;

    if (cutil_lsbmzmj.meld_only_need_num(tiles, history) === 0){
        case1 = 2;
    }
    ...
    for(var i = 0; i < tiles.length; i++){
        ...
        if (cutil_lsbmzmj.get_count(tiles, tiles[i]) === 1){
            idx = tmp.indexOf(tiles[i]);
            tmp.splice(idx, 1);
            case2 = Math.min(case2, 1 + cutil_lsbmzmj.meld_only_need_num(tmp, history));
        } else {
            idx = tmp.indexOf(tiles[i]);
            tmp.splice(idx, 1);
            idx = tmp.indexOf(tiles[i]);
            tmp.splice(idx, 1);
            case2 = Math.min(case2, cutil_lsbmzmj.meld_only_need_num(tmp, history));
        }
    }
...
cutil_lsbmzmj.meld_only_need_num = function(tiles, history, used){
    history = history || this.meld_history;
    used = used || 0;
    if (used > 4){
        return 999;
...
cutil_lsbmzmj.checkIs7Pairs = function(handTilesButKing, kingTilesNum){
    if (handTilesButKing.length + kingTilesNum !== 14) {return false}
    var tileDict = cutil_lsbmzmj.getTileNumDict(handTilesButKing);
    var need_num = 0
...
// 十三幺
cutil_lsbmzmj.getThirteenOrphans = function (handTilesButKing, kingTilesNum) {
    ...
    function withoutKings() {
        ...
        for (var i = 0; i < need_tiles.length; i++) {
            if (cutil_lsbmzmj.get_count(handTilesButKing, need_tiles[i]) === 2) {
                return true;
            }
        }
        return false;
...
cutil_lsbmzmj.classifyTiles = function(tiles, kingTiles){
    kingTiles = kingTiles || [];
    var kings = [];
    var chars = [];
    var bambs = [];
...
cutil_lsbmzmj.checkIs6Pairs = function(tilesButKing, kingTilesNum){
    if (tilesButKing.length + kingTilesNum !== 12) {return false}
    var tileDict = cutil_lsbmzmj.getTileNumDict(tilesButKing);
    var need_num = 0
    for(var tile in tileDict){
        need_num += (tileDict[tile] % 2);
    }
...
cutil_lsbmzmj.canNormalWinWithKing3N2 = function (handTilesButKing, kingTilesNum) {
    if ((handTilesButKing.length + kingTilesNum) % 3 !== 2) {
        return false;
    }
    var classified = cutil_lsbmzmj.classifyTiles(handTilesButKing, []);
...
cutil_lsbmzmj.canNormalWinWithKing3N = function (handTilesButKing, kingTilesNum) {
...
cutil_lsbmzmj.canNormalWinWithoutKing3N2 = function (handTiles) {
    if (handTiles.length % 3 !== 2) {
        return false;
    }
    var classified = cutil_lsbmzmj.classifyTiles(handTiles);
    var chars	= classified[1];
    var bambs	= classified[2];
    var dots	= classified[3];
    var winds	= classified[4];
...
cutil_lsbmzmj.isMeld = function (tiles) {
    if (tiles.length % 3 !== 0) {
        return false;
    }
...
cutil_lsbmzmj.isMeldWithPair = function (tiles) {
    if (tiles.length % 3 !== 2) {
        return false;
    }
    var total = 0;
...
cutil_lsbmzmj.checkMeldInPossible = function (tiles, possibleList) {
    var idx;
...
cutil_lsbmzmj.sortChowTileList = function (chowTile, tiles) {
...
cutil_lsbmzmj.tileSort = function (tiles, kingTiles) {
    var kings = kingTiles || [];
...
cutil_lsbmzmj.classifyKingTiles = function(tiles, kingTiles){
    kingTiles = kingTiles || [];
    var kings = [];
    var others = [];
    for (var i = 0; i < tiles.length; i++) {
...
//获取同样牌的张数 dict
cutil_lsbmzmj.getTileNumDict = function(tiles){
    var tileDict = {}
    for (var i = 0; i < tiles.length; i++) {
        var t = tiles[i]
        if (!tileDict[t]) {
...
cutil_lsbmzmj.count = function(list, key){
    var dict = {}
...
cutil_lsbmzmj.get_playing_room_detail = function (room_info) {
    var str_list = [];

    if(room_info['game_mode'] === 0) {
        str_list.push("普通玩法");
    } else if (room_info['game_mode'] === 1) {
        str_list.push("特殊牌型玩法");
    } else {
        str_list.push("耗子玩法");
    }
    ...
    if(room_info['reward'] === 0){
        str_list.push("非赏金");
    } else {
        str_list.push("赏金");
    }

    if(room_info['add_dealer'] === 1) {
        str_list.push("加庄");
    }
...
cutil_lsbmzmj.tileSortFunc = function(a, b){
	if(a == b){
		return 0;
	}
