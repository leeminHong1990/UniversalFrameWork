...
/*-----------------------------------------------------------------------------------------
 interface
 -----------------------------------------------------------------------------------------*/
var LSBMZMJGameRules = LSBMZMJRoomOperationAdapter.extend({
    ...
    getCanTingTiles:function () {
        ...
        for (var n = 0 ; n < handTiles.length ; n++) {
            ...
            for (var m = 0 ; m < canWinTiles.length ; m++) {
                ...
                if(canTingTiles.indexOf(handTiles[n])<0){
    ...
    checkCanTing:function (opDict) {
        //轮到自己，是否可以听牌
        opDict = opDict || {};
        if (this.curGameRoom.discardStateList[this.serverSitNum] === const_lsbmzmj.DISCARD_FREE) {
    ...
    getDrawOpDict: function (drawTile, serverSitNum) {
        ...
        if (this.isOpLimit()) {
            //胡
            if (handTiles.length % 3 === 2 && this.canWin(handTiles, drawTile, serverSitNum, const_val.OP_DRAW_WIN) && this.curGameRoom.discardStateList[serverSitNum] === const_lsbmzmj.DISCARD_FORCE) {
        ...
        for (var i = 0; i < handTiles.length; i++) {
            for (var j = 0; j < uptiles.length; j++) {
                ...
                if (upMeld.length === 3 && upMeld[0] === upMeld[1] && upMeld[1] === upMeld[2] && upMeld[0] === handTiles[i]) {
                	// if(this.curGameRoom.discardStateList[this.serverSitNum] == const_lsbmzmj.DISCARD_FORCE && !this.canContinueKong()){
                	if(this.curGameRoom.discardStateList[this.serverSitNum] == const_lsbmzmj.DISCARD_FORCE && upMeld[0] !== drawTile){
                		continue;
					}
                    if (!op_dict[const_val.OP_CONTINUE_KONG]) {
                        op_dict[const_val.OP_CONTINUE_KONG] = []
        ...
        // }
        //暗杠
        var tile2NumDict = cutil_lsbmzmj.getTileNumDict(handTiles)
        for (var key in tile2NumDict) {
            ...
            if (tile2NumDict[key] === 4) {
                if (this.curGameRoom.discardStateList[this.serverSitNum] === const_lsbmzmj.DISCARD_FORCE) {
    ...
    canWin:function(handTiles, finalTile, serverSitNum, aid){
        ...
        if(this.nomalCanWin(handTiles, finalTile, serverSitNum, aid)){
            //过胡只能自摸
            if (aid !== const_val.OP_DRAW_WIN && this.curGameRoom.pass_win_list[this.serverSitNum] === const_lsbmzmj.PASS_WIN) {
                return false;
            }
            // 不用算点数了
            return true;
    ...
    nomalCanWin: function (handTiles, finalTile, serverSitNum, aid) {
        ...
        var kingClassified = cutil_lsbmzmj.classifyKingTiles(handCopyTile, this.curGameRoom.kingTiles);
        ...
        // 没有癞子 7对 和 十三幺
        return cutil_lsbmzmj.canNormalWinWithoutKing3N2(handTilesButKing);

	    if (cutil_lsbmzmj.checkIs7Pairs(handTilesButKing, kingTilesNum)) {              // 7对
	        return true;
        } else if (this.curGameRoom.game_mode === const_lsbmzmj.SPECIAL_GAME_MODE && cutil_lsbmzmj.getThirteenOrphans(handTilesButKing, kingTilesNum)) { // 十三幺
            return true;
        } else if(kingTilesNum > 0){                                            // 有癞子
            return cutil_lsbmzmj.canNormalWinWithKing3N2(handTilesButKing, kingTilesNum);
        } else {                                                                // 没癞子
            return cutil_lsbmzmj.canNormalWinWithoutKing3N2(handTilesButKing);
        }
        // }
