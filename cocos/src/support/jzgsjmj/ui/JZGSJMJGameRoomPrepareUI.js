...
"use strict";
var JZGSJMJGameRoomPrepareUI = UIBase.extend({
	ctor: function () {
		this._super();
		this.resourceFilename = "res/ui/JZGSJMJGameRoomPrepareUI.json";
		this.talk_img_num = 0;
	},
	...
	initUI: function () {
		...
		if (this.curRound > 0 && player.curGameRoom.player_num === 3) {
		...
		var share_title = cutil.get_share_title(player.curGameRoom,const_val.JinZhongGSJMJ);

		//var share_desc = cutil_gsjmj.get_playing_room_detail(player.curGameRoom);
		var share_desc = cutil.get_share_desc(player.curGameRoom,const_val.JinZhongGSJMJ);
		...
		wxinvite_btn.addTouchEventListener(function (sender, eventType) {
			if (eventType == ccui.Widget.TOUCH_ENDED) {
				...
				share_title = cutil.get_share_title(player.curGameRoom,const_val.JinZhongGSJMJ);
