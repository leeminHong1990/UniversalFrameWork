...
"use strict";
var GSJMJGameRoomPrepareUI = UIBase.extend({
	ctor: function () {
		this._super();
		this.resourceFilename = "res/ui/GSJMJGameRoomPrepareUI.json";
		this.talk_img_num = 0;
	},
	...
	initUI: function () {
        ...
        var share_title = cutil.get_share_title(player.curGameRoom,const_val.GuaiSanJiaoMJ);

        //var share_desc = cutil_gsjmj.get_playing_room_detail(player.curGameRoom);
        var share_desc = cutil.get_share_desc(player.curGameRoom,const_val.GuaiSanJiaoMJ);
		...
		wxinvite_btn.addTouchEventListener(function (sender, eventType) {
			if (eventType == ccui.Widget.TOUCH_ENDED) {
                ...
                share_title = cutil.get_share_title(player.curGameRoom,const_val.GuaiSanJiaoMJ);
