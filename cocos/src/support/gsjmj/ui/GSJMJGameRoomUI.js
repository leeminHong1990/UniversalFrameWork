...
"use strict"
var GSJMJGameRoomUI = UIBase.extend({
	ctor: function () {
		this._super();
		this.talk_img_num = 0;
	},
	initUI: function () {
		...
		this.touch_state = const_gsjmj.PLAYER_TOUCH_SELF_STATE
	...
	_resetSelectTile: function () {
		...
		if (this.curSelectTile) {
			...
			this.update_canwin_tile_panel(const_gsjmj.NOT_DISPLAY_CANWIN_PANEL);
		}
	},
	...
	init_player_tile_panel: function () {
		...
		function unchoose_tile() {
			...
			self.update_canwin_tile_panel(const_gsjmj.NOT_DISPLAY_CANWIN_PANEL);
		}
		...
		function hand_tile_event(tile, touch_mode, sender) {
			...
			if (self.touch_state === const_gsjmj.PLAYER_TOUCH_SELF_STATE && !player.gameOperationAdapter.canDiscard(tile.tileNum)) {
				return
			}
			if (touch_mode == ccui.Widget.TOUCH_BEGAN) { // 只选中
				...
				if (self.touch_state !== const_gsjmj.PLAYER_TOUCH_SELF_STATE) { // 非出牌状态不能出牌
					return
				}
		...
		function touch_func(touch_pos, eventType, sender) {
			let effective_width = self.touch_state === const_gsjmj.PLAYER_TOUCH_SELF_STATE ? config.real_width * handTiles.length + offset_x : config.real_width * handTiles.length
			if (touch_pos.x >= 0 && touch_pos.x < effective_width && touch_pos.y >= 0 && touch_pos.y <= hand_panel_height) {
				...
				if (self.touch_state === const_gsjmj.PLAYER_TOUCH_SELF_STATE && touch_idx + 1 >= handTiles.length) { //3x+2 最后一张特殊处理
				...
				if (self.touch_state !== const_gsjmj.PLAYER_TOUCH_SELF_STATE && self.curSelectTile) {
					reset_tile()
				}
			}
		}
		...
		function player_hand_panel_event(sender, eventType) {
			// NOTE:下乡禁止触摸和滑动
			if (self.touch_state === const_gsjmj.PLAYER_TOUCH_FORCE_STATE) {
				if (self.curSelectTile) {
					reset_tile()
				}
				return;
			...
			if (eventType == ccui.Widget.TOUCH_BEGAN) {
				...
				touch_func(p, eventType, sender)
				if (self.curSelectTile && !self.moving_tile && self.touch_state === const_gsjmj.PLAYER_TOUCH_SELF_STATE && p.y - self.curSelectTile.getPositionY() > 76) { // 向上拖 生成一个 麻将子
					if (player.gameOperationAdapter.canDiscard(self.curSelectTile.tileNum)) { // 其他玩家打财神后限制出牌
				...
				if (self.moving_tile) {
					...
					// 出牌
					if (py > hand_panel_height + config.sel_posy && self.touch_state === const_gsjmj.PLAYER_TOUCH_SELF_STATE) {
	...
	mark_discard_king_tiles: function () {
		...
		for (var i = 0; i < list.length; i++) {
			...
			let start = 0;
			if (tileSum > const_gsjmj.MAX_DISCARD_TILES_SIZE) {
				start = tileSum - const_gsjmj.MAX_DISCARD_TILES_SIZE;
			}
	...
	mark_same_tile: function (tile_img) {
		tile_img.color = const_gsjmj.mark_same_color;
	},
	...
	cancel_mark_same_tiles: function () {
		...
		for (let i = 0; i < list.length; i++) {
			...
			for (let j = start; j < tileSum; j++) {
				if (player.curGameRoom.kingTiles.indexOf(tiles[j]) < 0) {
					let tile_img = cur_player_discard_panel.getChildByName("tile_img_" + j);
					this.cancel_tile_color(tile_img);
				}
			}
		}
	...
	lock_player_hand_tiles: function () {
		...
		var player = h1global.player();
		this.touch_state = const_gsjmj.PLAYER_TOUCH_OTHER_STATE;
		...
		function playAnimation() {
			...
			var step3 = cc.callFunc(function () {
				...
				if (myServerSitNum === curPlayerSitNum && (curGameRoom.handTilesList[player.serverSitNum].length) % 3 === 2) {
					...
					if (player) {
						self.update_operation_panel(player.gameOperationAdapter.getDrawOpDict(player.curGameRoom.lastDrawTile), const_val.SHOW_DO_OP)
					}
				} else {
	...
	play_discard_anim: function (serverSitNum, onlyTilePointer) {
		...
		if (discardTilesSum > const_gsjmj.MAX_DISCARD_TILES_SIZE) discardIndex = const_gsjmj.MAX_DISCARD_TILES_SIZE - 1;
	...
	update_player_discard_tiles: function (serverSitNum) {
		...
		var deskSize = const_gsjmj.DISCARD_TILES_SIZE;
		var maxDeskSize = const_gsjmj.MAX_DISCARD_TILES_SIZE;
	...
	update_wintips_btn: function () {
		...
		if (canWinTiles.length <= 0 || player.curGameRoom.curPlayerSitNum == player.serverSitNum) {
			...
			wintips_btn.addTouchEventListener(function (sender, eventType) {
				if (eventType === ccui.Widget.TOUCH_BEGAN) {
					self.update_canwin_tile_panel(const_gsjmj.WINTIPS_BTN_DISPLAY);
	...
	update_canwin_tile_panel: function (select_tile) {
		...
		var self = this;
		var canwin_tile_panel = self.rootUINode.getChildByName("canwin_tile_panel");
		if (select_tile == const_gsjmj.NOT_DISPLAY_CANWIN_PANEL) {
			canwin_tile_panel.setVisible(false);
			return;
		}
		var player = h1global.player();
	...
	play_result_anim: function (player_info_list) {
		...
		for (var i = 0; i < curGameRoom.player_num; i++) {
			...
			cutil_gsjmj.tileSort(handTiles, player.curGameRoom.kingTiles);
