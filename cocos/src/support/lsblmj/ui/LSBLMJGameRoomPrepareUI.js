...
"use strict";
var LSBLMJGameRoomPrepareUI = UIBase.extend({
	ctor:function() {
		this._super();
		this.resourceFilename = "res/ui/LSBLMJGameRoomPrepareUI.json";
		this.talk_img_num = 0;
	},
	...
	initUI:function(){
        ...
        var share_desc = cutil.get_share_desc(player.curGameRoom,const_val.LingShiBLMJ);
		...
		wxinvite_btn.addTouchEventListener(function(sender, eventType){
			if(eventType == ccui.Widget.TOUCH_ENDED){
                ...
                share_title = cutil.get_share_title(player.curGameRoom,const_val.LingShiBLMJ);
    ...
    get_share_desc : function () {
	    ...
	    // if (player.curGameRoom.same_suit_mode === 1) {
		 //    share_list.push("清一色");
	    // }
	    // if (player.curGameRoom.same_suit_loong === 1) {
		 //    share_list.push("清龙");
	    // }
	...
	playEmotionAnim:function(serverSitNum, eid){
        ...
        emotion.playEmotion(this.gameprepare_panel,eid,serverSitNum,true);
        return;//下面的是以前的代码
