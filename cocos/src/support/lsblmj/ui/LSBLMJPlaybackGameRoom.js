...
//
"use strict"
var LSBLMJPlaybackGameRoom = cc.Node.extend({
    ...
    doNext: function () {
        ...
        if (commands.length > 0) {
	        ...
	        if (command.aid === const_lsblmj.DISCARD_FORCE) {
                let player = h1global.player();
		        player.curGameRoom.discardStateList[command.serverSitNum] = const_lsblmj.DISCARD_FORCE;
        ...
        if (op_special_record) {
            ...
            while (flag) {
                ...
                if (re[3] == this.totalCommand - this.commands.length) {
                    ...
                    // player.postPlayerDiscardState(re[1], const_lsblmj.DISCARD_FORCE);
