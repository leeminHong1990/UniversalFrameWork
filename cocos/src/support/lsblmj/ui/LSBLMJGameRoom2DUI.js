...
var LSBLMJGameRoom2DUI = LSBLMJGameRoomUI.extend({
    className: "LSBLMJGameRoom2DUI",
    uiType: const_val.GAME_ROOM_2D_UI,
    ctor: function () {
        this._super();
        this.resourceFilename = "res/ui/LSBLMJGameRoom2DUI.json";
    ...
    get_play_discard_anim_config: function (serverSitNum, index, onlyTilePointer) {
        ...
        if(discardTileIdx > const_lsblmj.MAX_DISCARD_TILES_SIZE){
            discardTileIdx = Math.min(discardTileIdx , const_lsblmj.MAX_DISCARD_TILES_SIZE);
        }
