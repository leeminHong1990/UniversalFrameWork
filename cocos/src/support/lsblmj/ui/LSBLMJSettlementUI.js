"use strict"
var LSBLMJSettlementUI = SettlementUI.extend({
	...
	show_by_info: function (roundRoomInfo, serverSitNum, curGameRoom, confirm_btn_func, replay_btn_func) {
		...
		this.show(function(){
			...
			for(var i = 0; i < 4; i++){
				...
				self.update_player_win(i, server_seat_num, roundRoomInfo["win_idx"], roundRoomInfo["from_idx"], roundRoomInfo["dealer_idx"], roundRoomInfo["result_list"], curGameRoom.game_mode);
			}
	...
	show_rules : function (curGameRoom) {
		...
		share_list.push(curGameRoom.base_score + '倍');
	...
	update_player_hand_tiles:function(panel_idx, serverSitNum, curGameRoom ,tileList, win_idx, finalTile, concealedKongList){
		...
		if(win_idx == serverSitNum) {
			tileList.pop();
			cutil_lsbmzmj.tileSort(tileList, curGameRoom.kingTiles);
	...
	update_player_win:function(panel_idx, serverSitNum, win_idx, from_idx, dealer_idx, result, game_mode){
		var cur_player_info_panel = this.player_tiles_panels[panel_idx];
		...
		//0自摸 1点炮 2抢杠 3明杠 4暗杠 5庄家 6平胡 7凑一色 8清一色 9字一色 10边张 11砍张 12吊张 13门前清 14断幺（十三幺）15碰碰胡 16七小对 17一条龙 18龙套龙
		if(serverSitNum === win_idx) {
			var win_type = ["自摸", "点炮", "抢杠", "明杠", "暗杠", "庄家", "平胡", "凑一色", "清一色", "风一色", "边张", "砍张", "吊张", "门前清", "十三幺", "碰碰胡", "七小对", "一条龙", "龙套龙"];
			var win_type_score = ["*2", "+1", "+1", "+1", "+2", "+1", "+2", "+5", "+10", "+20", "+1", "+1", "+1", "", "+20", "+5", "+7", "+10", "+20"];
			for (var i = 0 ; i < result.length ; i++) {
				if (result[18] && (i === 8 || i === 17)) {continue;}
				if (result[i]) {
					item_win_type_label.string += win_type[i] + win_type_score[i] + "\t\t\t";
				}
			}
		}
