...
"use strict"
var LSBLMJGameRoomUI = UIBase.extend({
	ctor:function() {
		this._super();
		this.talk_img_num = 0;
	},
	initUI:function(){
		...
		if (h1global.curUIMgr.gameroominfo_ui) {
			...
			// if (player.curGameRoom.stand_four == 0) {
			// 	h1global.curUIMgr.gameroominfo_ui.setArrange(false);
			// } else {
			// 	h1global.curUIMgr.gameroominfo_ui.setArrange(true);
			// }
		}
	...
	init_ting_panel:function(){
		...
		function guo_btn_event(sender, eventType){
			if (eventType === ccui.Widget.TOUCH_ENDED) {
				...
				player.setDiscardState(const_lsblmj.DISCARD_FREE);
				h1global.curUIMgr.roomLayoutMgr.iterUI(function (ui) {
					ui.checkTing = false;
				});
	...
	show_ting_panel:function(is_show) {
		...
		var guo_btn = this.rootUINode.getChildByName("ting_operation_panel").getChildByName("guo_btn");
		...
		if (operation_panel.isVisible()) {
			...
			if (player.curGameRoom) {
				if (player.curGameRoom.stand_four == 1) {
					if (player.curGameRoom.handTilesList[player.serverSitNum].length == player.curGameRoom.standTileList[player.serverSitNum].length) {
	...
	_setTileMask:function () {
		...
		var standTileList = player.curGameRoom.standTileList[player.serverSitNum];
		var cur_player_hand_panel = this.rootUINode.getChildByName("player_tile_panel0").getChildByName("player_hand_panel");
		if (player.curGameRoom.stand_four == 1) {
			var temp_standTileList = cutil.deepCopy(standTileList);
			standTileList.sort(function (a, b) {
				return a - b;
			});
			var temp_handTilesList = cutil.deepCopy(handTiles);
			for (var i = 0; i < standTileList.length; i++) {
				temp_handTilesList.splice(temp_handTilesList.indexOf(standTileList[i]), 1);
			}
			if (this.isArrange) {
				handTiles = standTileList.concat(temp_handTilesList);
			}
			var tempCanTingTiles = player.gameOperationAdapter.canTingTiles.concat();
			for( var i = 0 ; i < handTiles.length ; i++) {
				var tile_img = cur_player_hand_panel.getChildByName("tile_img_" + i.toString());
				var index = tempCanTingTiles.indexOf(handTiles[i]);
				if ((i < temp_standTileList.length || this.standTileIdxList.indexOf(i) >= 0) && temp_standTileList.indexOf(handTiles[i]) >= 0 && index >= 0) {
					tile_img.color = const_lsblmj.mark_none_color;
					tempCanTingTiles[index] = -1;
					// temp_standTileList.splice(temp_standTileList.indexOf(handTiles[i]), 1);
				} else {
					tile_img.color = const_lsblmj.mark_same_color;
				}
			}
		} else {
			for( var i = 0 ; i < handTiles.length ; i++) {
				...
				if (player.gameOperationAdapter.canTingTiles.indexOf(handTiles[i]) < 0) {
					tile_img.color = const_tykddmj.mark_same_color;
				}
			}
		}
	},
    ...
    _resetSelectTile: function () {
        ...
        if (this.curSelectTile) {
            ...
            this.update_canwin_tile_panel(const_lsblmj.NOT_DISPLAY_CANWIN_PANEL);
        }
    },
	...
	init_player_tile_panel:function(){
		...
		var player = h1global.player();
		if (player.curGameRoom.stand_four == 1) {
			if (this.isArrange == undefined) {
				this.isArrange = true;
			} else if (this.isArrange == true) {
				this.isArrange = false;
			} else if (this.isArrange == false) {
				this.isArrange = true;
			}
		}
		...
		let standTileList = player.curGameRoom.standTileList[player.serverSitNum];
        ...
        function unchoose_tile() {
            ...
            self.update_canwin_tile_panel(const_lsblmj.NOT_DISPLAY_CANWIN_PANEL);
        }
        ...
        function discard_tile(tile, discard_pos) {
            ...
            if (player){
	            ...
	            if (self.checkTing && player.gameOperationAdapter.canTingTiles.indexOf(tile.tileNum) >= 0) {
		            player.setDiscardState(const_lsblmj.DISCARD_FORCE, tile.tileNum);
        ...
        function hand_tile_event(tile, touch_mode, sender){
			...
			if (self.touch_state === const_val.PLAYER_TOUCH_SELF_STATE && !player.gameOperationAdapter.canDiscardIdx(tile.index)){ //别人打财神后 限制出牌
            	return
			}
	        if (self.checkTing && player.gameOperationAdapter.canTingTiles.indexOf(tile.tileNum) < 0) {
		        return;
		...
		function touch_func(touch_pos, eventType, sender){
        	// cc.log(handTiles)
			if (player.curGameRoom.stand_four == 1 && self.isArrange) {
				...
				if (player.curGameRoom.standTileList[player.serverSitNum].length == player.curGameRoom.handTilesList[player.serverSitNum].length) {
					//这里特殊处理一下
					effective_width = effective_width - const_lsblmj.CLEARANCE * 2;
				}
				if (touch_pos.x >= -const_lsblmj.CLEARANCE && touch_pos.x < effective_width && touch_pos.y >= 0 && touch_pos.y <= hand_panel_height) {
					standTileList = player.curGameRoom.standTileList[player.serverSitNum];
					if (player.curGameRoom.stand_four == 1) {
						if (touch_idx < standTileList.length && self.checkTing !== true) {
							//立四麻将前四张牌不允许出
							return;
						}
						if (touch_idx < standTileList.length) {
							touch_pos.x = touch_pos.x + const_lsblmj.CLEARANCE;
							touch_idx = Math.floor(touch_pos.x / config.real_width)
						}
						var touch_idx_list = [];
						for (var i = 0; i < player.gameOperationAdapter.canTingTiles.length; i++) {
							for (var j = 0; j < player.curGameRoom.standTileList[player.serverSitNum].length; j++) {
								if (player.curGameRoom.standTileList[player.serverSitNum][j] === player.gameOperationAdapter.canTingTiles[i]) {
									if (touch_idx_list.indexOf(j) >= 0) {
										continue;
									}
									touch_idx_list.push(j);
								}
							}
						}
						if (self.checkTing && touch_idx_list.indexOf(touch_idx) < 0) {
							return;
						}
					}
			...
			} else if (player.curGameRoom.stand_four == 1 && self.isArrange == false){
				let effective_width = self.touch_state === const_val.PLAYER_TOUCH_SELF_STATE ? config.real_width * handTiles.length + offset_x : config.real_width * handTiles.length
				if (player.curGameRoom.standTileList[player.serverSitNum].length == player.curGameRoom.handTilesList[player.serverSitNum].length || self.standTileIdxList.indexOf(player.curGameRoom.handTilesList[player.serverSitNum].length - 1) >= 0) {
					//这里特殊处理一下
					effective_width = effective_width - const_lsblmj.CLEARANCE * 2;
				}
				if (touch_pos.x >= 0 && touch_pos.x < effective_width && touch_pos.y >= 0 && touch_pos.y <= hand_panel_height) {
					var touch_idx = Math.floor(touch_pos.x / config.real_width)
					if (self.standTileIdxList.indexOf(touch_idx) >= 0 && self.checkTing != true) {
						return;
					}
					if (self.checkTing && (self.standTileIdxList.indexOf(touch_idx) < 0 || player.gameOperationAdapter.canTingTiles.indexOf(player.curGameRoom.handTilesList[player.serverSitNum][touch_idx]) < 0)) {
						return;
					}
					if (self.touch_state === const_val.PLAYER_TOUCH_SELF_STATE && touch_idx + 1 >= handTiles.length) { //3x+2 最后一张特殊处理
						touch_idx = handTiles.length - 1
				...
				let effective_width = self.touch_state === const_tykddmj.PLAYER_TOUCH_SELF_STATE ? config.real_width * handTiles.length + offset_x : config.real_width * handTiles.length
				if (touch_pos.x >= 0 && touch_pos.x < effective_width && touch_pos.y >= 0 && touch_pos.y <= hand_panel_height) {
					var touch_idx = Math.floor(touch_pos.x / config.real_width)
					if (self.touch_state === const_tykddmj.PLAYER_TOUCH_SELF_STATE && touch_idx + 1 >= handTiles.length) { //3x+2 最后一张特殊处理
						touch_idx = handTiles.length - 1
						if (touch_idx * config.real_width + offset_x <= touch_pos.x && touch_pos.x < effective_width) {
							hand_tile_event(hand_list[touch_idx], eventType, sender)
						}
					...
					if (self.touch_state !== const_tykddmj.PLAYER_TOUCH_SELF_STATE && self.curSelectTile) {
						reset_tile()
					}
				}
			}
		...
		function player_hand_panel_event(sender, eventType){
            // NOTE:下乡禁止触摸和滑动
            if(self.touch_state === const_val.PLAYER_TOUCH_FORCE_STATE || player.curGameRoom.discardStateList[player.serverSitNum] === const_lsblmj.DISCARD_FORCE){
                if(self.curSelectTile){
                    reset_tile()
                }
                return;
			...
			if (eventType == ccui.Widget.TOUCH_BEGAN) {
				if (player.gameOperationAdapter.canTingTiles) {
					if (player.gameOperationAdapter.canTingTiles.length > 0) {
						if (self.curSelectTile && player.gameOperationAdapter.canTingTiles.indexOf(self.curSelectTile.tileNum) < 0 && player.curGameRoom.discardStateList[player.serverSitNum] === const_lsblmj.DISCARD_FORCE) {
							reset_tile()
							return;
						}
					}
		...
		if (player.curGameRoom.stand_four == 1 && this.isArrange) {
			var img = ccui.ImageView.create();
			img.setScale9Enabled(true);
			img.setTouchEnabled(true);
			img.setContentSize(cc.size(const_lsblmj.CLEARANCE, player_hand_panel.getContentSize().height));
			img.setAnchorPoint(0, 0);
			this.rootUINode.getChildByName("player_tile_panel0").addChild(img);
			img.setPosition(-const_lsblmj.CLEARANCE + player_hand_panel.getPositionX(), 0);
			img.addTouchEventListener(player_hand_panel_event);
		}
	...
	mark_king_tile:function (tile_img) {
		tile_img.color = const_lsblmj.mark_king_color;
    },
	...
	mark_prompt_tile:function (tile) {
        ...
        var cur_player_hand_panel = this.rootUINode.getChildByName("player_tile_panel0").getChildByName("player_hand_panel");
        var standTileList = player.curGameRoom.standTileList[player.serverSitNum];
		standTileList.sort(function (a, b) {return a - b;});
		var temp_handTilesList = cutil.deepCopy(handTiles);
		for (var i = 0 ; i < standTileList.length ; i++) {
			temp_handTilesList.splice(temp_handTilesList.indexOf(standTileList[i]), 1);
		}
	...
	mark_discard_king_tiles:function () {
        ...
        for(var i = 0; i < list.length; i++){
            ...
            let start = 0 ;
            if(tileSum > const_lsblmj.MAX_DISCARD_TILES_SIZE){
                start = tileSum - const_lsblmj.MAX_DISCARD_TILES_SIZE;
            }
	...
	mark_same_tiles:function(tileNum){
		...
		for(var i = 0; i < list.length; i++){
			...
			var maxDeskSize = const_lsblmj.MAX_DISCARD_TILES_SIZE;
			if (player.curGameRoom.discardStateList[i] === const_lsblmj.DISCARD_FORCE) {
	...
	cancel_mark_same_tiles:function(){
		...
		for(var i = 0; i < list.length; i++){
            ...
            for(var j = start; j< tileSum; j++){
                if(player.curGameRoom.kingTiles.indexOf(tiles[j]) < 0)	{
	                var tile_img = cur_player_discard_panel.getChildByName("tile_img_" + (j  - start));
	                if (tile_img.color.b === const_lsblmj.mark_ting_color.b) {
		                continue;
	                }
                    this.cancel_tile_color(tile_img);
                }
		...
		function playAnimation(){
			...
			var step3 = cc.CallFunc.create(function(){
				...
				if(myServerSitNum == curPlayerSitNum && (curGameRoom.handTilesList[myServerSitNum].length)%3==2){
					...
					// self.update_operation_panel(player.getDrawOpDict(curGameRoom.lastDrawTile), const_val.SHOW_DO_OP)
	...
	update_player_hand_tiles: function (serverSitNum, tileList) {
        ...
        var standTileList = player.curGameRoom.standTileList[serverSitNum];
        var tempStandTileList = cutil.deepCopy(standTileList);
        ...
        if (isPlayer && this.isArrange) {
	        standTileList.sort(function (a, b) {return a - b;});
	        var temp_handTilesList = cutil.deepCopy(handTilesList);
	        for (var i = 0 ; i < standTileList.length ; i++) {
		        temp_handTilesList.splice(temp_handTilesList.indexOf(standTileList[i]), 1);
	        }
	        handTilesList = standTileList.concat(temp_handTilesList);
        }
        for (var i = 0; i < 14; i++) {
            ...
            if(handTilesList[i] !== undefined){
                if(handTilesList[i] > 0){
                    ...
                    if(isPlayer){
	                    if(discardState === const_lsblmj.DISCARD_FORCE){
	                    	this.mark_same_tile(tile_img);
	                    	this.lock_player_hand_tiles();
	                    }else{
		                    if (tempStandTileList.indexOf(handTilesList[i]) >= 0) {
			                    this.mark_same_tile(tile_img);
			                    this.standTileIdxList.push(i);
			                    if (this.isArrange) {
				                    tile_img.setPositionX(tile_img.getPositionX() - const_lsblmj.CLEARANCE);
			                    }
			                    tempStandTileList.splice(tempStandTileList.indexOf(handTilesList[i]), 1);
		                    } else {
			                    this.cancel_tile_color(tile_img);
		                    }
	                       // this.cancel_tile_color(tile_img);
                    ...
                    if(isPlayer){
                        ...
                        tile_img.setVisible(true);
                        if (i < standTileList.length) {
	                        this.mark_same_tile(tile_img);
                        } else {
	                        this.cancel_tile_color(tile_img);
	                        if (!(handTilesList.length % 3 === 2 && i === handTilesList.length - 1)) {
		                        if (idx === 1) {
			                        tile_img.setPositionY(tile_img.getPositionY() + const_lsblmj.CLEARANCE);
		                        } else if (idx === 2) {
			                        tile_img.setPositionX(tile_img.getPositionX() - const_lsblmj.CLEARANCE);
		                        } else if (idx === 3) {
			                        tile_img.setPositionY(tile_img.getPositionY() - const_lsblmj.CLEARANCE);
        ...
        if(serverSitNum == player.serverSitNum && player.curGameRoom.discardStateList[serverSitNum] === const_lsblmj.DISCARD_FREE){
		...
		if (player.curGameRoom.stand_four == 1) {
			...
			h1global.curUIMgr.roomLayoutMgr.notifyObserver('mark_prompt_tile', this.op);
		}
	},
	...
	update_player_up_tiles: function (serverSitNum) {
        ...
        var isLeftShift = options.upTilesLeftShift;

		if (idx === 0 && isLeftShift) {
			cur_player_tile_panel.setPositionX(-const_lsblmj.CLEARANCE);
		}
        ...
        for (var i = 0; i < curUpNum; i++) {
            ...
            if (cur_upTilesList[i].length > 3) {
                ...
                if (cur_upTilesList[i][3]) {
                    ...
                    mahjong_img.setVisible(true);
                    if (player.curGameRoom.kingTiles == cur_upTilesList[i][3]) {
                        // var kingtilemark_img = this._createKingTileMark(kingtilemarkOptions);
                        // this.upTileMarksList[idx].push(kingtilemark_img);
                        // tile_img.addChild(kingtilemark_img);
	                    this.mark_king_tile(tile_img);
	...
	play_discard_anim:function(serverSitNum, onlyTilePointer, buckle){
        ...
        if(discardTilesSum > const_lsblmj.MAX_DISCARD_TILES_SIZE) discardIndex = const_lsblmj.MAX_DISCARD_TILES_SIZE - 1;
	...
	update_player_discard_tiles:function(serverSitNum){
		...
		var deskSize = const_lsblmj.DISCARD_TILES_SIZE;
		var maxDeskSize = const_lsblmj.MAX_DISCARD_TILES_SIZE;
		...
		var discardTilesList = player.curGameRoom.discardTilesList[serverSitNum];
		var tingTilePos = undefined;
		if (player.curGameRoom.discardStateList[serverSitNum] === const_lsblmj.DISCARD_FORCE) {
        ...
        for(var i = 0; i < deskSize; i++){
            ...
            if(i < maxDeskSize && discardTilesList[i] > 0){
	            ...
	            if (tingTilePos >= 0 && tingTilePos === i) {
                	if (isPlayer) {
		                // this.mark_same_tile(tile_img);
		                tile_img.color = const_lsblmj.mark_ting_color;
	                } else {
		                if (options.mahjongdesk) {
    ...
    update_wintips_btn:function () {
        ...
        if(canWinTiles.length <= 0 || player.curGameRoom.curPlayerSitNum == player.serverSitNum){
	        ...
	        wintips_btn.setPosition(cc.p(cc.winSize.width - 160.77, 208));
            wintips_btn.addTouchEventListener(function (sender, eventType) {
                if (eventType === ccui.Widget.TOUCH_BEGAN) {
                    self.update_canwin_tile_panel(const_lsblmj.WINTIPS_BTN_DISPLAY);
    ...
    update_canwin_tile_panel:function(select_tile){
        ...
        var self = this;
        var canwin_tile_panel = self.rootUINode.getChildByName("canwin_tile_panel");
        if(select_tile == const_lsblmj.NOT_DISPLAY_CANWIN_PANEL){
            canwin_tile_panel.setVisible(false);
            return;
        }
        var player = h1global.player();
        if (player.curGameRoom.stand_four == 1 && player.curGameRoom.standTileList[player.serverSitNum].indexOf(select_tile) < 0 && select_tile !== const_lsblmj.WINTIPS_BTN_DISPLAY) {
	        canwin_tile_panel.setVisible(false);
        	return;
        }
    ...
    play_result_anim: function (player_info_list) {
        ...
        for (var i = 0; i < curGameRoom.player_num; i++) {
			...
			cutil_lsblmj.tileSort(handTiles, player.curGameRoom.kingTiles);
    ...
    update_operation_panel:function(op_dict, from_type){
        ...
        var self = this;
        var player = h1global.player();
        if (player.curGameRoom.discardStateList[player.serverSitNum] === const_lsblmj.DISCARD_FORCE && player.curGameRoom.canWinCanSelectList[player.serverSitNum] == 0) {
        ...
        function op_btn_touch_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
	            ...
	            if (player.curGameRoom.stand_four == 1) {
		            h1global.curUIMgr.gameroom2d_ui.op = undefined;
		            h1global.curUIMgr.gameroom3d_ui.op = undefined;
	            }
	...
	update_arrange_tiles:function () {
		if (this.beginAnimPlaying) {return;}
		var player = h1global.player();
		this.init_player_tile_panel();
		this.update_player_hand_tiles(player.serverSitNum, player.curGameRoom.handTilesList[player.serverSitNum]);
	},
