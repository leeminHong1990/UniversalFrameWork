...
var LSBLMJGameRoomEntity = GameRoomEntity.extend({
	ctor: function (player_num, gameType) {
		...
		this.discardStateList = [const_lsblmj.DISCARD_FREE, const_lsblmj.DISCARD_FREE, const_lsblmj.DISCARD_FREE, const_lsblmj.DISCARD_FREE]
		...
		this.pass_win_list = [const_lsblmj.NO_PASS_WIN,const_lsblmj.NO_PASS_WIN,const_lsblmj.NO_PASS_WIN,const_lsblmj.NO_PASS_WIN]; //玩家是否过胡
  		...
  		for(var i = 0; i < recRoomInfo["player_advance_info_list"].length; i++){
  			...
  			for(var j = 0; j < curPlayerInfo["op_list"].length; j++){
  				...
  				if(op_info["opId"] === const_val.OP_PONG){
  					...
  					sortTiles = cutil_lsblmj.sortChowTileList(sortTiles[0], sortTiles);
                    // cutil_lsblmj.tileSort(sortTiles, this.kingTiles);
