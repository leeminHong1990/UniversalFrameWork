...
/*-----------------------------------------------------------------------------------------
 interface
 -----------------------------------------------------------------------------------------*/
var LSBLMJGameRules = LSBLMJRoomOperationAdapter.extend({

    getCanTingTiles:function () {
	    var handTiles = this.curGameRoom.handTilesList[this.serverSitNum].concat([]);
	    var standTileList = this.curGameRoom.standTileList[this.serverSitNum].concat([]);
	    var canTingTiles = [];
	    if (h1global.player().curGameRoom.stand_four == 1) {
		    for (var n = 0; n < standTileList.length; n++) {
			    var handTilesCopy = cutil.deepCopy(handTiles);
			    handTilesCopy.splice(handTilesCopy.indexOf(standTileList[n]), 1);
			    var canWinTiles = [];
			    ...
			    for (var m = 0; m < canWinTiles.length; m++) {
				    ...
				    canTingTiles.push(standTileList[n]);
				    break;
			    }
		    }
	    } else {
    ...
    getDrawOpDict: function (drawTile, serverSitNum) {
        ...
        if (this.isOpLimit()) {
            //胡
            if (handTiles.length % 3 === 2 && this.canWin(handTiles, drawTile, serverSitNum, const_val.OP_DRAW_WIN) && this.curGameRoom.discardStateList[serverSitNum] === const_lsblmj.DISCARD_FORCE) {
        ...
        for (var i = 0; i < handTiles.length; i++) {
            for (var j = 0; j < uptiles.length; j++) {
                ...
                if (upMeld.length === 3 && upMeld[0] === upMeld[1] && upMeld[1] === upMeld[2] && upMeld[0] === handTiles[i] && this.curGameRoom.discardStateList[this.serverSitNum] !== const_lsblmj.DISCARD_FORCE) {
	                if (this.curGameRoom.standTileList[this.serverSitNum].indexOf(handTiles[i]) >= 0 && this.curGameRoom.standTileList[this.serverSitNum].length === 1) {
	                    continue;
                    }
        ...
        //暗杠
        var tile2NumDict = cutil_lsblmj.getTileNumDict(handTiles)
        for (var key in tile2NumDict) {
            // if (this.curGameRoom.kingTiles.indexOf(eval(key)) >= 0 && this.curGameRoom.reward === 1){
            //     continue;
            // }
            if (tile2NumDict[key] === 4 && this.curGameRoom.discardStateList[this.serverSitNum] !== const_lsblmj.DISCARD_FORCE) {
                ...
                //     var p_tiles = cutil.deepCopy(handTiles).sort(function(a,b){return a-b;});
	             //    p_tiles.splice(p_tiles.indexOf(parseInt(key)), 1);
                //     var a_tiles = cutil.deepCopy(handTiles).sort(function(a,b){return a-b;});
	             ...
	             //    if (canWinTilesPre.sort(function(a,b){return a-b;}).toString() !== canWinTilesAft.sort(function(a,b){return a-b;}).toString()) {
		         //        continue;
                //     }
                // }
	            var standTileList = this.curGameRoom.standTileList[this.serverSitNum];
                if (standTileList.indexOf(parseInt(key)) >= 0 && cutil_lsblmj.get_count(standTileList, parseInt(key)) === standTileList.length) {
	                continue;
                }
	    ...
	    if (this.curGameRoom.discardStateList[this.serverSitNum] === const_lsblmj.DISCARD_FORCE && this.curGameRoom.canWinCanSelectList[this.serverSitNum] == 0) {
		    op_dict = {};
        }
        return op_dict
    },
    ...
    canWin:function(handTiles, finalTile, serverSitNum, aid){
        ...
        if(this.nomalCanWin(handTiles, finalTile, serverSitNum, aid)){
            //扣点点的规则限制列表
	        //如果最后一张是财神也ok
	        if(finalTile ==this.curGameRoom.kingTiles ){
    ...
    nomalCanWin: function (handTiles, finalTile, serverSitNum, aid) {
        ...
        var kingClassified = cutil_lsblmj.classifyKingTiles(handCopyTile, this.curGameRoom.kingTiles);
        var kings   = kingClassified[0];
        var handTilesButKing  = kingClassified[1];
        var kingTilesNum = kings.length;
	    var colorType = cutil_lsblmj.getTileColorType(handTiles, uptiles, this.curGameRoom.kingTiles);
	    if (cutil_lsblmj.getThirteenOrphans(handTilesButKing, kingTilesNum)) { // 十三幺
		    return true;
	    }
	    if (colorType === const_val.MIX_X_SUIT) {
		    return false;
	    }
	    if (cutil_lsblmj.checkIs7Pairs(handTilesButKing, kingTilesNum)) {              // 7对
            return true;
        } else if(cutil_lsblmj.canNormalWinWithoutKing3N2(handTilesButKing)) {        // 没癞子
		    return true;
        }
    },
