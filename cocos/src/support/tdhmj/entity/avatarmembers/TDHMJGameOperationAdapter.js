...
/*-----------------------------------------------------------------------------------------
												interface
-----------------------------------------------------------------------------------------*/
var TDHMJGameOperationAdapter = TDHMJPlaybackOperationAdapter.extend({
	...
	startGame : function(startInfo){
        ...
        if (this.serverSitNum == dealerIdx) {
			...
			cutil_tdhmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], kingTiles);
    ...
    removeHandTile: function (serverSitNum, tileNum, count , sort) {
        ...
        if(sort){
            cutil_tdhmj.tileSort(this.curGameRoom.handTilesList[serverSitNum], this.curGameRoom.kingTiles)
        }
    },
	...
	postOperation : function(serverSitNum, aid, tileList, buckle){
		...
		if(aid == const_val.OP_PASS){
			...
			if (h1global.curUIMgr &&  h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.isShow()) {
				...
				h1global.curUIMgr.roomLayoutMgr.notifyObserver("update_canwin_tile_panel", const_tdhmj.NOT_DISPLAY_CANWIN_PANEL);
			}
			// if(this.curGameRoom.kingTiles.indexOf(tileList[0]) >= 0){
			if(buckle === 1) {
				if(this.curGameRoom.playerInfoList[serverSitNum]["sex"] == 1){
			...
			if(this.serverSitNum == serverSitNum){
                ...
                cutil_tdhmj.tileSort(this.curGameRoom.handTilesList[this.serverSitNum], this.curGameRoom.kingTiles);
                var sortChowTiles = cutil_tdhmj.sortChowTileList(tileList[0], tileList.concat());
                // cutil_tdhmj.tileSort(sortChowTiles, this.curGameRoom.kingTiles);
	...
	doOperation : function(aid, tileList){
		...
		if(this.curGameRoom.curPlayerSitNum === this.serverSitNum && aid === const_val.OP_PASS){
			...
			if (this.checkPassWin) {
				this.setPassWinState(const_tdhmj.PASS_WIN);
				this.checkPassWin = false;
			}
			// return;
		}
