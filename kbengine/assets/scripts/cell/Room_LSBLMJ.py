...
import json
import const
from lsblmj.utility_lsblmj import *
...
class Room_LSBLMJ(Room):
	...
	def lsblmjBroadcastOperation2(self, idx, aid, tile_list = None):
		""" 将操作广播除了自己之外的其他人 """
		for i, p in enumerate(self.players_list):
			if p and i != idx:
				...
				p.postOperationLSBLMJ(idx, aid, tile_list, 0, standTileList)
	...
	def setPassWinStateLSBLMJ(self, avt_mb, state, tile):
		idx = -1
		DEBUG_MSG("room:{0},curround:{1} idx:{2} setPassWinStateLSBLMJ state:{3} tile:{4}".format(self.roomIDC, self.current_round, self.current_idx, state, tile))
		for i,p in enumerate(self.players_list):
		...
		if is_win:
			if 1 in result_list[6:17]:
				canWinSmall = True
			elif 1 in result_list[17:]:
				canWinBig = True
		...
		for t in tempCanWinTiles:
			...
			if win:
				if 1 in result[6:17] and canWinSmall and 1 not in result[17:]:
					p.canWinTiles.remove(t)
				elif 1 in result[17:] and canWinBig:
					p.canWinTiles.remove(t)
			tempTiles.remove(t)
		...
		for i,p in enumerate(self.players_list):
			if p is not None:
				p.postWinCanSelectLSBLMJ(idx, [p.canWinCanSelect for i,p in enumerate(self.players_list) if p is not None], [p.canWinTiles for i,p in enumerate(self.players_list) if p is not None])
	...
	def lsblmjBroadcastOperation(self, idx, aid, tile_list = None, buckle = 0):
		"""
		将操作广播给所有人, 包括当前操作的玩家
		...
		for i, p in enumerate(self.players_list):
			if p is not None:
				standTileList = [player.standTileList if player is not None and idx == i else [0] * len(player.standTileList) for idx,player in enumerate(self.players_list)]
				p.postOperationLSBLMJ(idx, aid, tile_list, buckle, standTileList)
...
# ----------------------------------------------------------------------------------------------------------------------
	...
	def getCanWinTiles(self, handTiles, win_op, idx, state = DISCARD_FREE):
		...
		for tile in allTiles:
			...
			small_result_list = result_list[6:17]
			big_result_list = result_list[17:]
			if is_win:
	...
	def can_win(self, handTiles, finalTile, win_op, idx, state = DISCARD_FREE):
		#"""0自摸 1点炮 2抢杠 3明杠 4暗杠 5庄家 6平胡 7凑一色 8清一色 9字一色 10边张 11砍张 12吊张 13门前清 14断幺（十三幺） 16七小对 15碰碰胡 17一条龙 18龙套龙"""
		result_list = [0] * 19
		base_score = 2
		result_list[6] = 1
		p = self.players_list[idx]
		...
		if kingTilesNum <= 0: 	#无财神(只要满足能胡就可以胡)
			if utility.meld_with_pair_need_num(handTilesButKing) <= kingTilesNum:
				...
				if checkIsOneDragon(handTilesButKing): # 一条龙
					result_list[17] = 1
					base_score += 10
				if result_list[8] == 1 and result_list[17] == 1: # 龙套龙
					result_list[18] = 1
	...
	def cal_win_score(self, idx, fromIdx, score, tile, aid, result):
		...
		for i, p in enumerate(self.players_list):
			if p and i != idx:
				if from_player.discard_state == DISCARD_FREE:
					from_player.add_score(win_player.kong_score_list[i] * kongMultiply)
				elif from_player.discard_state == DISCARD_FORCE:
					p.add_score(win_player.kong_score_list[i] * kongMultiply)
			elif p and i == idx:
				p.add_score(win_player.kong_score_list[i] * kongMultiply)
