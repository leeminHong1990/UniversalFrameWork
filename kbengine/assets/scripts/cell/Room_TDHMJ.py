...
class Room_TDHMJ(Room):
	...
	def tdhmjBroadcastOperation2(self, idx, aid, tile_list = None):
		""" 将操作广播除了自己之外的其他人 """
		for i, p in enumerate(self.players_list):
			if p and i != idx:
				p.postOperationTDHMJ(idx, aid, tile_list, 0)
	...
	def tdhmjBroadcastOperation(self, idx, aid, tile_list = None, buckle = 0):
		"""
		将操作广播给所有人, 包括当前操作的玩家
		...
		for i, p in enumerate(self.players_list):
			if p is not None:
				p.postOperationTDHMJ(idx, aid, tile_list, buckle)
	...
	def get_init_client_dict(self):
		return {
			...
			'add_winds'			: self.add_winds,
			'add_dealer'		: self.add_dealer,
			'kong_follow_win'	: self.kong_follow_win,
			'need_ting'			: self.need_ting,
			'bao_hu'			: self.bao_hu,
			'multiplayer_win'	: self.multiplayer_win,
			'king_mode'			: self.king_mode,
			'lack_door'			: self.lack_door,
...
# ----------------------------------------------------------------------------------------------------------------------
	...
	def canTenPai(self, handTiles):
		...
		for tile in handTiles:
			...
			for t in tryList:
				...
				tmp.append(t)
				sorted(tmp)
				if utility_tdhmj.isWinTile(tmp, self.kingTiles):
					result.append(t)
					return True
		return False
		# return result != []
	...
	def can_win(self, handTiles, finalTile, win_op, idx):
		#"""平胡 清一色 七小对 豪华七小对 一条龙 十三幺 明杠 暗杠 带庄 四个癞子"""
		result_list = [0] * 10
		base_score = 0
		...
		if self.game_mode == SPECIAL_GAME_MODE:
			if utility_tdhmj.getThirteenOrphans(handTilesButKing, kingTilesNum):
				result_list[5] = 1
				base_score += 20
		...
		#2N
		# result_list = [0] * 9
		is7Pair, isBaoTou, kongNum = utility_tdhmj.checkIs7Pair(handCopyTiles, handTilesButKing, kingTilesNum, self.kingTiles, finalTile)
		if is7Pair:
			# DEBUG_MSG("room:{},curround:{} is7Pair isBaoTou:{} kongNum:{}".format(self.roomIDC, self.current_round, isBaoTou, kongNum))
			result_list[2] = 1 # 七小对
			if self.game_mode == SPECIAL_GAME_MODE:
				base_score += 5
				if utility_tdhmj.getTileColorType(handTilesButKing, uptiles, self.kingTiles) == const.SAME_SUIT:
					result_list[1] = 1 # 清一色
					base_score += 5
			if kongNum > 0:
		...
		if kingTilesNum <= 0: 	#无财神(只要满足能胡就可以胡)
			...
			if utility.meld_with_pair_need_num(handTilesButKing) <= kingTilesNum:
				colorType = utility_tdhmj.getTileColorType(handTilesButKing, uptiles, self.kingTiles)
				...
				if self.game_mode == SPECIAL_GAME_MODE:
					if colorType == const.SAME_SUIT:
						result_list[1] = 1 # 清一色
						base_score += 5
					if utility_tdhmj.checkIsOneDragon(handTilesButKing, kingTilesNum, self.kingTiles):
						result_list[4] = 1 # 一条龙
						base_score += 5
			...
			if utility.winWith3N2NeedKing(handTilesButKing) <= kingTilesNum:
				colorType = utility_tdhmj.getTileColorType(handTiles, uptiles, self.kingTiles)
				if self.lack_door == 1 and colorType == MIX_X_SUIT:
					# DEBUG_MSG("room:{},curround:{} colorType = MIX_X_SUIT handTiles:{} finalTile:{} win_op:{} idx:{}".format(self.roomIDC, self.current_round, handTiles, finalTile, win_op, idx))
					return False, base_score, result_list
				if self.game_mode == SPECIAL_GAME_MODE:
					if colorType == const.SAME_SUIT:
						result_list[1] = 1 # 清一色
						base_score += 5
					if utility_tdhmj.checkIsOneDragon(handTiles, kingTilesNum, self.kingTiles):
						result_list[4] = 1 # 一条龙
						base_score += 5
					if result_list[4] == 0 and result_list[1] == 0:
						result_list[0] = 1
			...
			if self.bao_hu == 1:
				if self.add_dealer == 1 and from_player.discard_state == DISCARD_FORCE: # 带庄
					...
					if aid == const.OP_DRAW_WIN:
						...
						win_player.add_score(1)
			elif self.bao_hu != 1 and self.add_dealer == 1: # 带庄
				if aid == const.OP_DRAW_WIN:
					dealer_player.add_score(-2)
					win_player.add_score(2)
				elif aid != const.OP_DRAW_WIN:
			...
			if aid == const.OP_EXPOSED_KONG or aid == const.OP_CONTINUE_KONG: # 明杠 碰后接杠
				...
				for i, p in enumerate(self.players_list):
					if p and i != idx:
						...
						kongPlayer.kong_score_list[i] -= 3
			elif aid == const.OP_CONCEALED_KONG:
				pass
