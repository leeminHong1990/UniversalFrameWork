...
import switch
import utility
from llkddmj.utility_llkddmj import *
from llkddmj.const_llkddmj import *
from Functor import Functor
from KBEDebug import *
from Room import Room

class Room_LLKDDMJ(Room):
	...
	def broadcastOperationLLKDDMJ(self, idx, aid, tile_list = None, buckle = 0):
		"""
		将操作广播给所有人, 包括当前操作的玩家
		...
		for i, p in enumerate(self.players_list):
			if p is not None:
				p.postOperationLLKDDMJ(idx, aid, tile_list, buckle)
	...
	def setPassWinState(self, avt_mb, state):
		...
		for i,p in enumerate(self.players_list):
			if p is not None and p == avt_mb:
				idx = i
				break
		DEBUG_MSG("room:{0},curround:{1} idx:{2} setPassWinState state:{3}".format(self.roomIDC, self.current_round, idx, state))
	...
	def setDiscardState(self, avt_mb, state, tile):
		...
		if not self.can_change_discard_state(p.tiles, idx, state):
			...
			return
		p.setDiscardStateLLKDDMJ(state)
	...
	def waitForOperation(self, idx, aid, tile, nextIdx = -1): #  aid抢杠 杠花没人可胡 nextIdx还是自己
		...
		if sum([len(x) for x in notifyOpList]) > 0:
			...
			# 摸打模式
			# for i,p in enumerate(self.players_list):
			# 	if p is not None and len(notifyOpList[i]) > 0 and p.discard_state == DISCARD_FORCE:
			# 		wait_op_dict = None
			...
			# 			if self.game_mode != KING_GAME_MODE and (wait_op_dict["aid"] >> 3) != const.SHOW_KONG:
...
# ----------------------------------------------------------------------------------------------------------------------
		...
		if prefabHandTiles is not None:
			for i, p in enumerate(self.players_list):
				if p is not None and len(prefabHandTiles) >= 0:
					p.tiles = prefabHandTiles[i] if len(prefabHandTiles[i]) <= INIT_TILE_NUMBER else \
					prefabHandTiles[i][0:INIT_TILE_NUMBER]
	...
	def rollKingTile(self, prefabKingTiles):
		...
		if prefabKingTiles is not None and len(prefabKingTiles) > 0:
			if self.king_num > 0:
				...
				elif self.king_mode == 1:  # 随机耗子
					self.kingTiles.append(prefabKingTiles[0])
				elif self.king_mode == 2:  # 双耗子
					if len(prefabKingTiles) >= 2:
						self.kingTiles.append(prefabKingTiles[0])
						self.kingTiles.append(prefabKingTiles[1])
					else:
						self.kingTiles.append(const.DRAGON_GREEN)
						self.kingTiles.append(const.DRAGON_WHITE)
		else:
			if self.king_num > 0:
				...
				if self.king_mode == 0:  # 风耗子
					...
					for tile in list(self.tiles):
						if tile not in roll_list:
							roll_list.append(tile)
				...
				if self.king_mode == 2:  # 双耗子
					firstKingTile = self.kingTiles[0]
					secendKingTile = 0
					for i in range(self.king_num):
						if firstKingTile < const.BOUNDARY:
							secendKingTile = firstKingTile - 8 if (firstKingTile + 1) % 10 == 0 else firstKingTile + 1
						elif firstKingTile in list(const.WINDS):
							if firstKingTile + 1 not in list(const.WINDS):
								secendKingTile = firstKingTile - 3
							else:
								secendKingTile = firstKingTile + 1
						elif firstKingTile in list(const.DRAGONS):
							if firstKingTile + 1 not in list(const.DRAGONS):
								secendKingTile = firstKingTile - 2
							else:
								secendKingTile = firstKingTile + 1
						else:
							DEBUG_MSG("rollKingTile error!")
						self.kingTiles.append(secendKingTile)
	...
	def canTenPai(self, handTiles, idx):
						...
						if player.discard_state == DISCARD_FORCE:
							...
							for winTile in canWinTilesAft:
								if winTile in self.kingTiles:
									# if self.game_mode == KING_GAME_MODE and self.ting_mouse == 1:
	...
	def can_win(self, handTiles, finalTile, win_op, idx, isCheckEKong = False):
		...
		p = self.players_list[idx]
		if p.discard_state == DISCARD_FREE:
		...
		if finalTile in self.kingTiles:
			for winTile in self.canWinTiles:
				if winTile in self.kingTiles:
					if self.game_mode == KING_GAME_MODE and self.ting_mouse == 1 and (winTile > const.BOUNDARY or winTile % 10 >= 6):
						...
						if sorted(canWinCopyTiles) != sorted(self.kingTiles):
							...
							if sorted(self.kingTiles)[0] % 10 == 1 and sorted(self.kingTiles)[-1] % 10 == 9 and kingTilesNum == 1 and finalTile in self.kingTiles and len(handTilesButKingcopyc) % 3 == 2 and utility.meld_with_pair_need_num(handTilesButKingcopyc) <= 0:
								pass
							else:
								continue
						if kingTilesNum <= 1:
		...
		if self.game_mode == SPECIAL_GAME_MODE:
			if getThirteenOrphans(handTilesButKing, kingTilesNum):
				...
				return True, multiplyCalc(self.game_mode, win_op, result_list, base_score, is_ekongwin, self.special_mul), result_list
		...
		if kingTilesNum > 0 and self.mouse_general == 1:
			if utility.winWith3N2NeedKing(handTilesButKing) <= kingTilesNum:
				result_list[0] = 1
				tileIsMouseGeneral = isMouseGeneral(handTiles, handTilesButKing, kingTilesNum, self.kingTiles, finalTile)
				if tileIsMouseGeneral:
					firstIsMouseGeneral = True

		# 2N
		result_list = [0] * 6
		is7Pair, isBaoTou, kongNum = checkIs7Pair(handCopyTiles, handTilesButKing, kingTilesNum, self.kingTiles,
														  finalTile)
		if is7Pair and not firstIsMouseGeneral and ((self.game_mode == KING_GAME_MODE and self.seven_pair == 1) or self.game_mode != KING_GAME_MODE):
			# DEBUG_MSG(
			# 	"room:{},curround:{} is7Pair isBaoTou:{} kongNum:{}".format(self.roomIDC, self.current_round, isBaoTou,
			# 																kongNum))
			# if not isBaoTou or self.king_mode != DOUBLE_KING_MODE::
			result_list[2] = 1
			if kongNum > 0:
				result_list[3] = 1
		...
		if kingTilesNum <= 0:  # 无财神(只要满足能胡就可以胡)
			...
			if utility.meld_with_pair_need_num(handTilesButKing) <= kingTilesNum:
				if self.game_mode == SPECIAL_GAME_MODE:
					if utility.getTileColorType(handTilesButKing, uptiles) == const.SAME_SUIT:
						result_list[1] = 1
					elif checkIsOneDragon(handTilesButKing):
						result_list[4] = 1
				else:
					result_list[0] = 1
			...
			if utility.winWith3N2NeedKing(handTilesButKing) <= kingTilesNum:
				...
				if self.mouse_general == 1:
					if self.is_win_limit(finalTile, win_op) == False and self.mouse_general_onetwo == 0:
						return False, base_score, result_list
					tileIsMouseGeneral = isMouseGeneral(handTiles, handTilesButKing, kingTilesNum, self.kingTiles,
															finalTile)
					if tileIsMouseGeneral and win_op != const.OP_DRAW_WIN:
						DEBUG_MSG(
							"room:{},curround:{} 3N win_op:{} tileIsMouseGeneral:{}".format(self.roomIDC, self.current_round,
																						win_op, tileIsMouseGeneral))
						return False, base_score, result_list
					elif tileIsMouseGeneral and win_op == const.OP_DRAW_WIN:
						if not isCheckEKong:
							self.lastWinScore = 20
						self.isMouseWin = True
						return True, 20, result_list
	...
	def cal_win_score(self, idx, fromIdx, score, tile, aid):
		...
		if self.players_list[fromIdx].discard_state == DISCARD_FREE:
				...
				if idx == self.dealer_idx:
					for i, p in enumerate(self.players_list):
						if p and i != idx:
							...
							p.add_score(20 * score_mul)
				elif idx != self.dealer_idx:
					self.players_list[self.dealer_idx].add_score(-20)
					self.players_list[idx].add_score(20)
			elif aid != const.OP_DRAW_WIN:
				if idx == self.dealer_idx:
					if self.players_list[fromIdx].discard_state == DISCARD_FREE:
						self.players_list[fromIdx].add_score(-10 * score_mul)
						self.players_list[idx].add_score(10 * score_mul)
					if self.players_list[fromIdx].discard_state == DISCARD_FORCE:
						for i, p in enumerate(self.players_list):
							if p and i != idx:
								p.add_score(-10)
							elif p and i == idx:
					...
					if fromIdx == self.dealer_idx:
						if self.players_list[fromIdx].discard_state == DISCARD_FREE:
							self.players_list[fromIdx].add_score(-10 * score_mul)
							self.players_list[idx].add_score(10 * score_mul)
						elif self.players_list[fromIdx].discard_state == DISCARD_FORCE:
							self.players_list[fromIdx].add_score(-10)
							self.players_list[idx].add_score(10)
					elif self.players_list[fromIdx].discard_state == DISCARD_FREE:
						self.players_list[fromIdx].add_score(-10)
						self.players_list[idx].add_score(10)
					elif self.players_list[fromIdx].discard_state == DISCARD_FORCE:
	...
	def red_score(self, idx, fromIdx, aid, tile):
		...
		if tile > const.BOUNDARY:
			...
			base_score = tile % 10
		if tile in self.kingTiles and self.game_mode == KING_GAME_MODE:
			base_score = 50
		if aid == const.OP_EXPOSED_KONG:  # 明杠
			if self.players_list[fromIdx].discard_state == DISCARD_FREE and self.bao_kong != 1:
