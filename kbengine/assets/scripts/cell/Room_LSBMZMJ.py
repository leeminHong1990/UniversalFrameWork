...
import switch
import utility
from lsbmzmj.utility_lsbmzmj import *
from lsbmzmj.const_lsbmzmj import *
from Functor import Functor
from KBEDebug import *
from Room import Room

class Room_LSBMZMJ(Room):
	...
	def broadcastOperationLSBMZMJ(self, idx, aid, tile_list = None, buckle = 0):
		"""
		将操作广播给所有人, 包括当前操作的玩家
		...
		for i, p in enumerate(self.players_list):
			if p is not None:
				p.postOperationLSBMZMJ(idx, aid, tile_list, buckle)
	...
	def paySuccessCbk(self):
		...
		if switch.DEBUG_BASE == 0:
			...
			prefabKingTiles = []
			prefabHandTiles = [
				[9, 9, 9, 4, 5, 6, 35, 36, 37, 58, 59, 57, 31],
				[31,9,1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4],
				[35],
				[]
				# [],
				# [],
			...
			prefabTopList	= [51,52,53,54,55,56,34]
	...
	def setDiscardState(self, avt_mb, state, tile):
		...
		if not self.can_change_discard_state(p.tiles, idx, state):
			...
			return
		p.setDiscardStateLSBMZMJ(state)
	...
	def winGame(self, idx, op, finalTile, from_idx, score, result):
		...
		# if self.players_list[from_idx].discard_state == DISCARD_FREE:
		# 	self.dealer_idx = from_idx
		# else:
		# if op == const.OP_KONG_WIN or op == const.OP_GIVE_WIN:
	...
	def doOperation(self, avt_mb, aid, tile_list):
		...
		if aid == const.OP_DISCARD:
			...
			if is_win:
				p.draw_winca(tile, score, result)
			else:
...
# ----------------------------------------------------------------------------------------------------------------------
	...
	def can_win(self, handTiles, finalTile, win_op, idx):
		...
		# ERROR_MSG("last_player_idx", self.last_player_idx)
		last_p = self.players_list[self.last_player_idx]
		p = self.players_list[idx]
		if p.discard_state == DISCARD_FREE:
			return False, base_score, result_list
		if last_p.discard_state == DISCARD_FREE and win_op != const.OP_DRAW_WIN:
		...
		# 		return True, (base_score + multiplyCalc(self.game_mode, win_op, result_list)) * \
		# 			   [2 if win_op == const.OP_DRAW_WIN else 1][0], result_list
		# 2N
		result_list = [0] * 6
	...
	def cal_win_score(self, idx, fromIdx, score, tile, aid):
		...
		if idx == self.dealer_idx:
			DEBUG_MSG("idx == {0} is a dealer".format(idx))
			for i, p in enumerate(self.players_list):
				if p and i != idx:
					p.add_score(-score * 2)
				elif p and i == idx:
