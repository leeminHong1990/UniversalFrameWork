...
class Adapter(object):
	...
	# 出牌状态
	def setDiscardStateLLKDDMJ(self, state):
		DEBUG_MSG("setDiscardStateLLKDDMJ idx:{0},state:{1}".format(self.idx, state))
	...
	def flowRound(self):
		""" 流局 """
		self.score = 0
		exposed_kong_num = sum([1 for op in self.op_r if op[0] == const.OP_EXPOSED_KONG])
		concealed_kong_num = sum([1 for op in self.op_r if op[0] == const.OP_CONCEALED_KONG])
		continue_kong_num = sum([1 for op in self.op_r if op[0] == const.OP_CONTINUE_KONG])
		self.continue_kong = self.continue_kong - continue_kong_num
		self.exposed_kong = self.exposed_kong - exposed_kong_num
		self.concealed_kong = self.concealed_kong - concealed_kong_num
	...
	def chow(self, tile_list):
		...
		self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_CHOW, tile_list)
	#碰
	def pong(self, tile):
		""" 碰 """
		...
		self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_PONG, [tile, tile, tile])
	...
	def concealedKong(self, tile):
		...
		if self.discard_state == DISCARD_FORCE:
			self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_CONCEALED_KONG, [0, 0, 0, tile])
		else:
	...
	def exposedKong(self, tile):
		...
		self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_EXPOSED_KONG, [tile] * 4)
	...
	def continueKong(self, tile):
		...
		self.room.last_player_idx = self.idx
		if self.discard_state == DISCARD_FORCE:
			self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_CONTINUE_KONG, [tile] * 4)
		else:
	...
	def draw_win(self, tile, score, result):
		...
		if self.discard_state == DISCARD_FORCE:
			self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
		else:
	...
	def kong_win(self, tile, score, result):
		...
		for upTile in last_player.upTiles:
			...
			if t == tile and len(upTile) > 3:
				upTile.remove(tile)
		# last_player.upTiles[-1].remove(tile)
		self.room.red_score(last_player.kong_record_list[-1][1], last_player.kong_record_list[-1][2], last_player.kong_record_list[-1][0], last_player.kong_record_list[-1][3][0])
		# if self.discard_state == DISCARD_FORCE:
		self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_KONG_WIN, [self.room.players_list[self.room.current_idx].last_draw,])
		# else:
		# 	self.room.broadcastOperation2(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
		self.room.winGame(self.idx, const.OP_KONG_WIN, tile, self.room.last_player_idx, score, result)

	def give_win(self, tile, score, result, fromIdx = -1):
		""" 放炮胡 """
		if fromIdx == -1:
			self.tiles.append(tile)
			fromIdx = self.room.last_player_idx
		self.win_times += 1
		if result[1] > 0:
			self.bao_tou_win_times += 1
			if len(result) > 4 and result[-1] > 0:
		...
		self.op_r.append((const.OP_GIVE_WIN, [tile,], fromIdx))
		self.last_op = const.OP_GIVE_WIN
		self.room.op_record.append((const.OP_GIVE_WIN, self.idx, fromIdx, [tile,]))
		# if self.discard_state == DISCARD_FORCE:
		# 	def delay_callback():
		# 		self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_GIVE_WIN, [tile, ])
		...
		# else:
		self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_GIVE_WIN, [tile, ])
		self.room.winGame(self.idx, const.OP_GIVE_WIN, tile, fromIdx, score, result)
		# self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_GIVE_WIN, [tile,])
		# self.room.winGame(self.idx, const.OP_GIVE_WIN, tile, self.room.last_player_idx, score, result)
	...
	def forceDiscard(self):
		if self.room.can_discard(self.idx, self.last_draw):
			DEBUG_MSG("room:{},curround:{} Player:{} forceDiscard:{}".format(self.room.roomIDC, self.room.current_round, self.idx, self.discard_state))
			self.room.all_discard_tiles.append(self.last_draw)
			self.discardTile(self.last_draw)
		else:
			DEBUG_MSG("room:{},curround:{} Player:{} can't force discard this tile:{}".format(self.room.roomIDC, self.room.current_round, self.idx, self.last_draw))
	...
	def cutTile(self, tile):
		...
		self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_CUT, [tile,])
	...
	def discardTile(self, tile = None):
		...
		# if self.room.pass_win_list[self.idx] == 1:
		# 	if self.pass_win_isself == False:
		# 		self.room.pass_win_list[self.idx] = 0
		# 	elif self.pass_win_isself == True:
		# 		self.pass_win_isself = False
		...
		if self.discard_state == DISCARD_FORCE:
			...
			self.room.broadcastOperationLLKDDMJ(self.idx, const.OP_DISCARD, [tile, ], self.buckle)
		else:
