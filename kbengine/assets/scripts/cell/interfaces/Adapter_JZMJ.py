...
class Adapter(object):
	...
	# 出牌状态
	def setDiscardStateJZMJ(self, state):
		DEBUG_MSG("setDiscardStateJZMJ idx:{0},state:{1}".format(self.idx, state))
	...
	def chow(self, tile_list):
		...
		self.room.jzmjBroadcastOperation(self.idx, const.OP_CHOW, tile_list)
	#碰
	def pong(self, tile):
		""" 碰 """
		...
		self.room.jzmjBroadcastOperation(self.idx, const.OP_PONG, [tile, tile, tile])
	...
	def concealedKong(self, tile):
		...
		if self.discard_state == DISCARD_FORCE:
			self.room.jzmjBroadcastOperation(self.idx, const.OP_CONCEALED_KONG, [0, 0, 0, tile])
		else:
			self.room.jzmjBroadcastOperation2(self.idx, const.OP_CONCEALED_KONG, [0, 0, 0, tile])
		def delay_callback():
	...
	def exposedKong(self, tile):
		...
		self.room.jzmjBroadcastOperation(self.idx, const.OP_EXPOSED_KONG, [tile] * 4)
	...
	def continueKong(self, tile):
		...
		self.room.last_player_idx = self.idx
		if self.discard_state == DISCARD_FORCE:
			self.room.jzmjBroadcastOperation(self.idx, const.OP_CONTINUE_KONG, [tile] * 4)
		else:
			self.room.jzmjBroadcastOperation2(self.idx, const.OP_CONTINUE_KONG, [tile] * 4)
		def delay_callback():
	...
	def kongWreath(self, tile):
		...
		self.room.jzmjBroadcastOperation2(self.idx, const.OP_KONG_WREATH, [tile, ])
	...
	def draw_win(self, tile, score, result):
		...
		if self.discard_state == DISCARD_FORCE:
			self.room.jzmjBroadcastOperation(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
		else:
			self.room.jzmjBroadcastOperation2(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
	...
	def kong_win(self, tile, score, result):
		...
		# if self.discard_state == DISCARD_FORCE:
		self.room.jzmjBroadcastOperation(self.idx, const.OP_KONG_WIN, [self.room.players_list[self.room.current_idx].last_draw,])
		# else:
		# 	self.room.jzmjBroadcastOperation2(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
	...
	def give_win(self, tile, score, result):
		...
		# if self.discard_state == DISCARD_FORCE:
		# 	def delay_callback():
		# 		self.room.jzmjBroadcastOperation(self.idx, const.OP_GIVE_WIN, [tile, ])
		...
		if self.discard_state == DISCARD_FORCE:
			...
			if canWinBig:
				...
				for i,p in enumerate(self.room.players_list):
					if p is not None:
						p.postWinCanSelectJZMJ(self.idx, [p.canWinCanSelect for i,p in enumerate(self.room.players_list) if p is not None], [p.canWinTiles for i,p in enumerate(self.room.players_list) if p is not None])
			if not is_first:
				self.room.jzmjBroadcastOperation2(self.idx, const.OP_DRAW, [0,])
				...
				self.postOperationJZMJ(self.idx, const.OP_DRAW, [tile,], 0, standTileList)
	...
	def cutTile(self, tile):
		...
		self.room.jzmjBroadcastOperation(self.idx, const.OP_CUT, [tile,])
	...
	def discardTile(self, tile = None):
		...
		if self.discard_state == DISCARD_FORCE:
			...
			self.room.jzmjBroadcastOperation(self.idx, const.OP_DISCARD, [tile, ], self.buckle)
		else:
			self.room.jzmjBroadcastOperation2(self.idx, const.OP_DISCARD, [tile, ]) #没倒计时模式广播给所有人
