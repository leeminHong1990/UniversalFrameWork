...
class Adapter(object):
	...
	# 出牌状态
	def setDiscardStateLSBLMJ(self, state):
		DEBUG_MSG("setDiscardStateLSBLMJ idx:{0},state:{1}".format(self.idx, state))
	...
	def chow(self, tile_list):
		...
		self.room.lsblmjBroadcastOperation(self.idx, const.OP_CHOW, tile_list)
	#碰
	def pong(self, tile):
		""" 碰 """
		...
		self.room.lsblmjBroadcastOperation(self.idx, const.OP_PONG, [tile, tile, tile])
	...
	def concealedKong(self, tile):
		...
		if self.discard_state == DISCARD_FORCE:
			self.room.lsblmjBroadcastOperation(self.idx, const.OP_CONCEALED_KONG, [0, 0, 0, tile])
		else:
			self.room.lsblmjBroadcastOperation2(self.idx, const.OP_CONCEALED_KONG, [0, 0, 0, tile])
		def delay_callback():
	...
	def exposedKong(self, tile):
		...
		self.room.lsblmjBroadcastOperation(self.idx, const.OP_EXPOSED_KONG, [tile] * 4)
	...
	def continueKong(self, tile):
		...
		self.room.last_player_idx = self.idx
		if self.discard_state == DISCARD_FORCE:
			self.room.lsblmjBroadcastOperation(self.idx, const.OP_CONTINUE_KONG, [tile] * 4)
		else:
			self.room.lsblmjBroadcastOperation2(self.idx, const.OP_CONTINUE_KONG, [tile] * 4)
		def delay_callback():
	...
	def kongWreath(self, tile):
		...
		self.room.lsblmjBroadcastOperation2(self.idx, const.OP_KONG_WREATH, [tile, ])
	...
	def draw_win(self, tile, score, result):
		...
		if self.discard_state == DISCARD_FORCE:
			self.room.lsblmjBroadcastOperation(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
		else:
			self.room.lsblmjBroadcastOperation2(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
	...
	def kong_win(self, tile, score, result):
		...
		# if self.discard_state == DISCARD_FORCE:
		self.room.lsblmjBroadcastOperation(self.idx, const.OP_KONG_WIN, [self.room.players_list[self.room.current_idx].last_draw,])
		# else:
		# 	self.room.lsblmjBroadcastOperation2(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
	...
	def give_win(self, tile, score, result):
		...
		# if self.discard_state == DISCARD_FORCE:
		# 	def delay_callback():
		# 		self.room.lsblmjBroadcastOperation(self.idx, const.OP_GIVE_WIN, [tile, ])
	...
	def drawTile(self, tile, is_first = False):
		...
		if self.discard_state == DISCARD_FORCE:
			...
			big_result_list = result[17:]
			canWinBig = False
			if 1 in big_result_list:
				canWinBig = True
			if canWinBig:
				self.canWinCanSelect = 0
				for i,p in enumerate(self.room.players_list):
					if p is not None:
						p.postWinCanSelectLSBLMJ(self.idx, [p.canWinCanSelect for i,p in enumerate(self.room.players_list) if p is not None], [p.canWinTiles for i,p in enumerate(self.room.players_list) if p is not None])
			if not is_first:
				self.room.lsblmjBroadcastOperation2(self.idx, const.OP_DRAW, [0,])
				...
				self.postOperationLSBLMJ(self.idx, const.OP_DRAW, [tile,], 0, standTileList)
			if is_win and self.canWinCanSelect == 0:
	...
	def cutTile(self, tile):
		...
		self.room.lsblmjBroadcastOperation(self.idx, const.OP_CUT, [tile,])
	...
	def discardTile(self, tile = None):
		...
		if self.discard_state == DISCARD_FORCE:
			...
			self.room.lsblmjBroadcastOperation(self.idx, const.OP_DISCARD, [tile, ], self.buckle)
		else:
			self.room.lsblmjBroadcastOperation2(self.idx, const.OP_DISCARD, [tile, ]) #没倒计时模式广播给所有人
