...
class Adapter(object):
	...
	# 出牌状态
	def setDiscardStateLSBMZMJ(self, state):
		DEBUG_MSG("setDiscardStateLSBMZMJ idx:{0},state:{1}".format(self.idx, state))
	...
	def chow(self, tile_list):
		...
		self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_CHOW, tile_list)
	#碰
	def pong(self, tile):
		""" 碰 """
		...
		self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_PONG, [tile, tile, tile])
	...
	def concealedKong(self, tile):
		...
		if self.discard_state == DISCARD_FORCE:
			self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_CONCEALED_KONG, [0, 0, 0, tile])
		else:
	...
	def exposedKong(self, tile):
		...
		self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_EXPOSED_KONG, [tile] * 4)
	...
	def continueKong(self, tile):
		...
		self.room.last_player_idx = self.idx
		if self.discard_state == DISCARD_FORCE:
			self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_CONTINUE_KONG, [tile] * 4)
		else:
	...
	def draw_win(self, tile, score, result):
		...
		if self.discard_state == DISCARD_FORCE:
			self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
		else:
	...
	def kong_win(self, tile, score, result):
		...
		# if self.discard_state == DISCARD_FORCE:
		self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_KONG_WIN, [self.room.players_list[self.room.current_idx].last_draw,])
		# else:
		# 	self.room.broadcastOperation2(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
	...
	def give_win(self, tile, score, result):
		...
		# if self.discard_state == DISCARD_FORCE:
		# 	def delay_callback():
		# 		self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_GIVE_WIN, [tile, ])
	...
	def drawTile(self, tile, is_first = False):
		...
		if not is_first:
			...
			self.postOperationLSBMZMJ(self.idx, const.OP_DRAW, [tile,])
		if self.discard_state == DISCARD_FORCE:
	...
	def cutTile(self, tile):
		...
		self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_CUT, [tile,])
	...
	def discardTile(self, tile = None):
		...
		if self.discard_state == DISCARD_FORCE:
			...
			self.room.broadcastOperationLSBMZMJ(self.idx, const.OP_DISCARD, [tile, ], self.buckle)
		else:
