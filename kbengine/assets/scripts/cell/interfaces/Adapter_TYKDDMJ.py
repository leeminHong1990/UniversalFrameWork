...
class Adapter(object):
	...
	def __init__(self, idx):
		...
		# 出牌状态
		self.discard_state = const_tykddmj.DISCARD_FREE
		# 出耗子的次数
	...
	# 出牌状态
	def setDiscardStateTYKDDMJ(self, state):
		DEBUG_MSG("setDiscardStateTYKDDMJ idx:{0},state:{1}".format(self.idx, state))
	...
	def chow(self, tile_list):
		...
		self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_CHOW, tile_list)
	#碰
	def pong(self, tile):
		""" 碰 """
		...
		self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_PONG, [tile, tile, tile])
	...
	def concealedKong(self, tile):
		...
		if self.discard_state == const_tykddmj.DISCARD_FORCE:
			self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_CONCEALED_KONG, [0, 0, 0, tile])
		else:
		...
		self.room.add_timer(const_tykddmj.DELAY_OP_FORCE_KONG_DRAW, delay_callback)
	...
	def exposedKong(self, tile):
		...
		self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_EXPOSED_KONG, [tile] * 4)
	...
	def continueKong(self, tile):
		...
		self.room.last_player_idx = self.idx
		if self.discard_state == const_tykddmj.DISCARD_FORCE:
			self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_CONTINUE_KONG, [tile] * 4)
		else:
	...
	def draw_win(self, tile, score, result):
		...
		if self.discard_state == const_tykddmj.DISCARD_FORCE:
			self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
		else:
	...
	def kong_win(self, tile, score, result):
		...
		# if self.discard_state == const_tykddmj.DISCARD_FORCE:
		self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_KONG_WIN, [self.room.players_list[self.room.current_idx].last_draw,])
		# else:
		# 	self.room.broadcastOperation2(self.idx, const.OP_DRAW_WIN, [self.last_draw,])
	...
	def give_win(self, tile, score, result, fromIdx = -1):
		...
		# if self.discard_state == const_tykddmj.DISCARD_FORCE:
		# 	def delay_callback():
		# 		self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_GIVE_WIN, [tile, ])
		...
		# 	self.room.add_timer(const_tykddmj.DELAY_OP_FORCE, delay_callback)
		# else:
		self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_GIVE_WIN, [tile, ])
	...
	def drawTile(self, tile, is_first = False):
		...
		if not is_first:
			...
			self.postOperationTYKDDMJ(self.idx, const.OP_DRAW, [tile,])
		if self.discard_state == const_tykddmj.DISCARD_FORCE:
			is_win, score, result = self.room.can_win(list(self.tiles), tile, const.OP_DRAW_WIN, self.idx)
			if is_win and self.room.game_mode != const_tykddmj.KING_GAME_MODE:
				pass
				# self.room.add_timer(1, Functor.Functor(self.draw_win, tile, score, result))
			elif is_win and self.room.game_mode == const_tykddmj.KING_GAME_MODE:
				pass
		...
		# elif self.discard_state == const_tykddmj.DISCARD_FREE:
	...
	def cutTile(self, tile):
		...
		self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_CUT, [tile,])
	...
	def discardTile(self, tile = None):
		...
		if self.discard_state == const_tykddmj.DISCARD_FORCE:
			self.buckle += 1
			if self.buckle == 1:
				self.room.op_record.append((const_tykddmj.DISCARD_FORCE, self.idx, self.idx, [tile,]))
				self.room.tingTileList[self.idx] = len(self.discard_desk_tiles) - 1
			self.room.broadcastOperationTYKDDMJ(self.idx, const.OP_DISCARD, [tile, ], self.buckle)
		else:
