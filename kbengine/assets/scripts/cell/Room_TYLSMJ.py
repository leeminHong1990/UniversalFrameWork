...
class Room_TYLSMJ(Room):
	...
	def tylsmjBroadcastOperation2(self, idx, aid, tile_list = None):
		""" 将操作广播除了自己之外的其他人 """
		for i, p in enumerate(self.players_list):
			if p and i != idx:
				...
				p.postOperationTYLSMJ(idx, aid, tile_list, 0, standTileList)
	...
	def tylsmjBroadcastOperation(self, idx, aid, tile_list = None, buckle = 0):
		"""
		将操作广播给所有人, 包括当前操作的玩家
		...
		for i, p in enumerate(self.players_list):
			if p is not None:
				standTileList = [player.standTileList if player is not None and idx == i else [0] * len(player.standTileList) for idx,player in enumerate(self.players_list)]
				p.postOperationTYLSMJ(idx, aid, tile_list, buckle, standTileList)
	...
	def get_init_client_dict(self):
		return {
			...
			'same_suit_mode'	: self.same_suit_mode,
			'same_suit_loong'	: self.same_suit_loong,
...
# ----------------------------------------------------------------------------------------------------------------------
	...
	def canTenPai(self, handTiles):
		...
		for tile in handTiles:
			...
			for t in tryList:
				...
				tmp.append(t)
				sorted(tmp)
				if utility_tylsmj.isWinTile(tmp, self.kingTiles):
					result.append(t)
					return True
		return False
		# return result != []
	...
	def can_pong(self, idx, t):
		...
		if len(standTileList) == 1 and t in standTileList and tiles.count(t) <= 2:
			return False
		# if t in self.kingTiles:
		# 	return False
	...
	def can_win(self, handTiles, finalTile, win_op, idx):
		...
		if kingTilesNum <= 0: 	#无财神(只要满足能胡就可以胡)
			if utility.meld_with_pair_need_num(handTilesButKing) <= kingTilesNum:
				...
				if p.idx == self.dealer_idx: # 庄家
					result_list[5] = 1
					base_score += 1
				result_list[6], result_list[7], result_list[8] = utility_tylsmj.checkIsEdgeMidSingle(handTilesButKing, finalTile)
				...
				if result_list[6] == 1 or result_list[7] == 1 or result_list[8] == 1: # 边张 砍张 吊将
					base_score += 1
				colorType = utility_tylsmj.getTileColorType(handTilesButKing, uptiles)
				if colorType == LACK_DOOR: # 缺门
					result_list[9] = 1
					base_score += 1
				if self.same_suit_mode == 1 and colorType == const.SAME_SUIT: # 清一色
					result_list[10] = 1
					base_score += 10
				if utility_tylsmj.checkIsOneDragon(handTilesButKing): # 一条龙
					result_list[11] = 1
					base_score += 10
		...
		# 		result_list[0] = 1
		# 		DEBUG_MSG("room:{},curround:{} 3N win_op:{} finalTile:{}".format(self.roomIDC, self.current_round, win_op, finalTile))
		...
		# 			DEBUG_MSG("room:{},curround:{} 3N win_op:{} isMouseGeneral:{}".format(self.roomIDC, self.current_round, win_op, isMouseGeneral))
		# 			return False , base_score, result_list
