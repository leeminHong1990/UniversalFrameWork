		...
		try:
			...
			def timeout_cb():
				if not sock._closed:
					...
					sock.close()
					INFO_MSG("AsyncRequest timeout closing sock")
				if task and task[2]:
					task[2](None)
					INFO_MSG("AsyncRequest timeout call task(None)")
			...
			try:
				...
				if task and task[2]:
					...
					if resp.getcode() / 100 == 2:
						...
						task[2](None)
						INFO_MSG("AsyncRequest code != 200 call task(None)")
			except:
				self.logsError()
				if task and task[2]:
					task[2](None)
					INFO_MSG("AsyncRequest ERROR call task(None)")
			finally:
