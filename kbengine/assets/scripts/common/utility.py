...
def calc_cost(game_type, params):
	if game_type in const.GameTypeTotal:
		...
		if game_type == const.DouDiZhu:
			base = int(game_round / 4)
			AAbase = int(game_round / 8)
		elif game_type == const.LvLiang7:
			base = int(game_round / 2)
			AAbase = 1
		else:
			base = int(game_round / 4) + 1
			AAbase = int(game_round / 8)

		player_num = params.get('player_num', None)
		if (game_type == const.LvLiangKDDMJ or game_type == const.TaiYuanKDDMJ) and player_num and player_num == 2:
			base = base - 1
