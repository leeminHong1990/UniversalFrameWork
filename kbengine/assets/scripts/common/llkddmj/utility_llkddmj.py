...
def multiplyCalc(game_mode, win_op, result_list, base_score, is_ekongwin, special_mul):
	sBase_score = 0
	if game_mode == SPECIAL_GAME_MODE:
		if special_mul == 1:
			sBase_score = base_score
			if result_list[3] == 1:
				sBase_score = base_score * 3
			elif result_list[1] == 1 or result_list[2] == 1 or result_list[4] == 1 or result_list[5] == 1:
				sBase_score = base_score
			else:
				sBase_score = 0
		else:
	...
	win_aid = win_op
	if is_ekongwin:
		win_aid = const.OP_GIVE_WIN
	last_score = (base_score + sBase_score) * [2 if win_aid == const.OP_DRAW_WIN else 1][0]
	return last_score
